...
Interact directly with the Meta AI on WhatsApp through the Z-API, sending text messages to get automated and contextual responses.

---

:::warning Attention

Currently, the Meta AI in Z-API supports **only text messages**. This means that audio, documents, images, and other multimedia files are not compatible in this integration. Therefore, only text messages should be sent to ensure proper functioning.

Additionally, this feature is available **only for personal WhatsApp accounts**, i.e., it is not accessible for business accounts.
...
- **Virtual Assistants**: Create intelligent assistants for customer service
- **Quick Research**: Conduct research directly on WhatsApp without opening browsers
- **Image Generation**: Use the "Imagine" functionality to create images from text descriptions
- **Personalized Suggestions**: Receive restaurant, travel destination, and activity recommendations

---

## For No-Code Users

If you are using an automation platform like n8n or Make, use the endpoint `/send-text` with the number `13135550002` to interact with Meta AI.
...
This method allows direct interaction with the Meta AI on WhatsApp through the Z-API, facilitating the sending of text messages to get automated and contextual responses.

With it, you can send questions and commands to the Meta AI in private conversations, directing the messages to the exclusive number **13135550002**, or include the AI in groups using the group ID as recipient.

### How to Use

To start using the Meta AI with Z-API, simply send a text message to the Meta AI number **`13135550002`** using the endpoint [`/send-text`](/docs/messages/texto-simples).
...
| Attribute | Type | Description |
|----------|------|-----------|
| `delayMessage` | number | Delay in seconds (1~15 sec). Default delay if not specified is 1~3 sec |
| `delayTyping` | number | Delay in seconds for "Typing..." status (1~15 sec). Default delay if not specified is 0 |
| `mentioned` | array | Pass the number `13135550002` if you want to trigger Meta AI in a group |
...
| Attribute | Type | Description |
|----------|------|-----------|
| `zaapId` | string | Unique ID of the message in the Z-API system |
| `messageId` | string | ID of the message on WhatsApp |
...
[Webhook on receiving message](/docs/webhooks/ao-receber) - Receive Meta AI responses through the webhook

---

## <Icon name="Lightbulb" size="md" /> Tips {#tips}

- **Meta AI Number**: Always use `13135550002` for private chats
- **Only Text**: Remember that only text messages are supported
- **Personal Accounts Only**: This feature is available only for personal WhatsApp accounts, not business
- **Groups**: To use in groups, mention the number `13135550002` in the array `mentioned`
- **"Imagine" Functionality**: Use text descriptions to generate images

---

## Next Steps

- [Meta AI - Introduction](/docs/meta-ai/introducao) - Learn more about Meta AI
- [Send Simple Text](/docs/messages/texto-simples) - Full documentation of the endpoint
