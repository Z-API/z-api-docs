---
description: Practical tips, troubleshooting, and best practices for using the Z-API
  efficiently.
sidebar_position: 0
title: Tips and Troubleshooting
---

import styles from './index.module.css';

## <Icon name="Lightbulb" size="lg" /> Tips and Troubleshooting

<div className={styles.heroSection}>

Practical tips, troubleshooting, and best practices for using the Z-API efficiently. Learn how to avoid common issues and optimize your integrations.

</div>

---

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShieldAlert" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Blocking and Bans</h3>
  </div>
  <div className={styles.cardDescription}>
    Understand how to avoid blocking and bans from WhatsApp, and what to do if this happens.
  </div>
  <a href="/docs/tips/bloqueios-e-banimentos" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShieldAlert" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Blocking and Bans 2025</h3>
  </div>
  <div className={styles.cardDescription}>
    Updated guide for 2025 on how to avoid blocking and bans from WhatsApp.
  </div>
  <a href="/docs/tips/bloqueios-e-banimentos-2025" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MousePointerClick" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Button Functionality</h3>
  </div>
  <div className={styles.cardDescription}>
    Understand how interactive buttons work in WhatsApp messages.
  </div>
  <a href="/docs/tips/funcionamento-botoes" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="IdCard" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>LID</h3>
  </div>
  <div className={styles.cardDescription}>
    Learn about LID (Linked ID) and how to use this unique identifier from WhatsApp.
  </div>
  <a href="/docs/tips/lid" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Code2" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Postman Collection</h3>
  </div>
  <div className={styles.cardDescription}>
    Import the official Z-API collection in Postman to quickly test all endpoints.
  </div>
  <a href="/docs/tips/colecao-postman" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Timer" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>File Expiration</h3>
  </div>
  <div className={styles.cardDescription}>
    Understand how file expiration works in WhatsApp and how to manage it.
  </div>
  <a href="/docs/tips/expiracao-arquivos" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Scale" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Z-API vs Official API</h3>
  </div>
  <div className={styles.cardDescription}>
    Compare the Z-API with the official WhatsApp Business API and understand the differences.
  </div>
  <a href="/docs/tips/z-api-vs-api-oficial" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Phone" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enabling Fixed Numbers</h3>
  </div>
  <div className={styles.cardDescription}>
    Learn how to enable fixed numbers in WhatsApp for use with the Z-API.
  </div>
  <a href="/docs/tips/habilitar-numero-fixo" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Smartphone" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Emulators</h3>
  </div>
  <div className={styles.cardDescription}>
    Tips on using Android emulators with the Z-API and best practices.
  </div>
  <a href="/docs/tips/emuladores" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Zap" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Stability</h3>
  </div>
  <div className={styles.cardDescription}>
    Tips to maintain stability and avoid frequent disconnections.
  </div>
  <a href="/docs/tips/estabilidade" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Power" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Always Active Android</h3>
  </div>
  <div className={styles.cardDescription}>
    Configure your Android device to keep WhatsApp always active and connected.
  </div>
  <a href="/docs/tips/android-sempre-ativo" className={styles.cardLink}>
    See guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Lock" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Screen Lock</h3>
  </div>
  <div className={styles.cardDescription}>
    Configure screen lock to keep WhatsApp active even when the device is locked.
  </div>
  <a href="/docs/tips/bloqueio-tela" className={styles.cardLink}>
    See guide →
  </a>
</div>

</div>

---

:::tip Best Practices

Follow these tips to ensure that your integrations with the Z-API are stable and efficient!

:::