...
# <Icon name="Ban" size="lg" /> Block or Unblock Contact

Block or unblock a contact on WhatsApp through the Z-API.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituation}

This method allows you to block or unblock a contact on WhatsApp. When a contact is blocked, they cannot send messages to you and you do not receive notifications from them.
...
|----------|------|------------|
| `phone` | string | Phone number to block or unblock in the format DDI DDD NUMBER. Ex: `551199999999`. **IMPORTANT**: Send only numbers, without formatting or mask |
| `action` | string | Action to be performed: `"block"` to block or `"unblock"` to unblock |
...
|-------|------|------------|
| `value` | boolean | `true` if the action was successful, `false` in case of failure |
...
| `400` | Invalid parameters | Check if `action` is `"block"` or `"unblock"` and if the number is in the correct format |

---

## <Icon name="Info" size="md" /> Notes {#observacoes}

- **Action**: Use `"block"` to block or `"unblock"` to unblock
- **Effect**: When blocked, the contact cannot send messages and you do not receive notifications
- **Reversible**: You can unblock at any time using `"unblock"`
- **Format**: The number should be in the format DDI DDD NUMBER, without formatting