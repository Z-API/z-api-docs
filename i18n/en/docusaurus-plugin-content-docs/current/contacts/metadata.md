...
# <Icon name="Info" size="lg" /> Metadata Contact

Get detailed information about a specific contact, including name, profile photo, message and other public information.

---

## <Icon name="Info" size="md" /> Concept {#conceituacao}

This method returns the metadata information of a specific contact. The information includes name, profile photo, message and other public information available on WhatsApp.

:::tip Tip about contact image
If you intend to store the contact's image, note that we always return in `get-contacts` the attribute `imgUrl` with it for you. However, it is important to remember that this becomes available for only **48 hours**. After this period, the image link is deleted by WhatsApp itself.

We recommend that, if you need to update your contact's image, you use the method [Get Contact Image](/docs/contacts/imagem).
:::
...
| `name` | string | **Full name** of the contact. Only returns filled if you have the number in your contacts |
| `short` | string | **Name** of the contact. Only returns filled if you have the number in your contacts |
| `vname` | string | Name of the Vcard of the contact, if it has one |
| `notify` | string | Name configured in WhatsApp settings |
| `imgUrl` | string | URL of the contact's photo. **WhatsApp deletes it after 48h** |
| `about` | string | Message from the contact's profile |
...
| `415` | Missing Content-Type | For GET requests, it's not necessary, but check if you're sending a body |
| `401` | Invalid token | Check the header `Client-Token` |
| `404` | Contact not found | Check if the number is correct and if the instance is connected |

---

## <Icon name="Info" size="md" /> Notes {#observacoes}

- **Full name and short**: Only return filled if you have the number saved in your contacts
- **imgUrl**: The image link expires after 48 hours. Use the method [Get Contact Image](/docs/contacts/imagem) to get an updated link
- **vname**: Returns the Vcard name if the contact has it configured
- **notify**: Returns the name configured in WhatsApp, even if it's not in your contacts