...
# <Icon name="UserMinus" size="lg" /> Remove Contacts

Remove contacts from your WhatsApp contact list through the Z-API.

---

## <Icon name="Info" size="md" /> Concept {#conceituacao}

This method is responsible for removing WhatsApp contacts from your mobile device's contact list.

:::caution About This Resource
The method to remove contacts from the WhatsApp list will only work for accounts that have already received the necessary update. Make sure your **WhatsApp** account has received the update before using this resource. Otherwise, the operation will not be completed successfully.
:::
...
## <Icon name="FileText" size="md" /> Request Body {#corpo-requisicao}

The request body must be a JSON array containing the phone numbers of contacts to be removed (only strings).

__PROTECTED_bd6d8543-c09a-4ef6-bcbd-9595eb32dd3b__

### <Icon name="Settings" size="sm" /> Parameters {#parametros}

| Field | Type | Required | Description |
|:------ |:----- |:---------- |:------------------------------------------------------------------------ |
| Array | array of strings | Yes | Array of phone numbers in the format DDI + DDD + NUMBER (ex: `["554499999999", "554499998888"]`). **IMPORTANT**: Send only numbers, without formatting or mask |
...
| Code | Reason | Tip |
|--------|--------|------|
| 405 | Method Not Allowed | Check if you sent DELETE correctly as specified at the beginning of this topic. |
| 415 | Unsupported Media Type | Make sure to add "Content-Type" as "application/json" in the request headers |
...
|-------|------|-------------|
| `success` | boolean | `true` if the removal was successful, `false` in case of failure |
| `errors` | array | Array of errors, if any (only when `success` is `false`) |
...
|--------|--------|---------------|
| `400` | Empty array or invalid numbers | Check if the array contains valid numbers in the correct format (DDI DDD NUMBER) |
| `405` | Incorrect HTTP method | Make sure to use `DELETE` as specified |
...
- **Required Update**: This method will only work for accounts that have already received the necessary WhatsApp update
- **Format**: Numbers must be in DDI DDD NUMBER format, without formatting or mask
- **Array**: The request body must be a JSON array of phone numbers
- **Multiple Contacts**: You can remove multiple contacts in a single request