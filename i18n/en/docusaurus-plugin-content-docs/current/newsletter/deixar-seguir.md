...
This method is responsible for making the instance stop following a newsletter.
...
Make sure you are sending the correct specification of the method, that is, verify if you sent POST, PUT or GET as specified at the beginning of this topic.
...
curl_easy_setopt(hnd, CURLOPT_URL, "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter");
...
(client/put "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter" {:headers {:client-token "{{security-token}}"}
                                                                                                :content-type :json
...
```csharp
var client = new RestClient("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter");
...
```http
PUT /instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter HTTP/1.1
...
```java
HttpResponse<String> response = Unirest.put("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter")
...
```js
fetch("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter", {
  "method": "PUT",
  "headers": {
    "client-token": "{{security-token}}"
  },
...
xhr.open("PUT", "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter");
...
var options = {
  ...
  "port": null,
  "path": "/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter",
  "headers": {
    "client-token": "{{security-token}}"
  }
};
...
var req = unirest("PUT", "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter");
...
NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter"]
...
let uri = Uri.of_string "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter" in
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter",
...
$request->setUrl('https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter');
...
$request->setRequestUrl('https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter');
...
$response = Invoke-WebRequest -Uri 'https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter' -Method PUT -Headers $headers -ContentType 'undefined' -Body '{"id": "ID do Canal"}'
```
...
$headers.Add("content-type", "application/json")
$headers.Add("client-token", "{{security-token}}")
$response = Invoke-RestMethod -Uri 'https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter' -Method PUT -Headers $headers -ContentType 'undefined' -Body '{"id": "ID do Canal"}'
```
...
conn.request("PUT", "/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter", payload, headers)
...
url = URI("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter")
...
```bash
echo '{"id": "ID do Canal"}' |  \
  http PUT https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter \
  client-token:'{{security-token}}'
```
...
let request = NSMutableURLRequest(url: NSURL(string: "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/unfollow-newsletter")! as URL,
