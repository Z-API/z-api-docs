...
This method is responsible for making the instance follow a newsletter.
...
```json
{
  "id": "12345678@newsletter"
}
```

:::warning
The newsletter ID should always include the suffix "@newsletter", as this is the standard used by WhatsApp itself.
:::
...
Make sure you are sending the correct method specification, that is, verify if you sent POST, PUT or GET as specified at the beginning of this topic.

## 415

If you receive a 415 error, make sure to add in the request headers the "Content-Type" of the object you are sending, for the most part "application/json"
...
curl_easy_setopt(hnd, CURLOPT_URL, "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter");
...
(client/put "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter" {:headers {:client-token "{{security-token}}"}
                                                                                              :content-type :json
...
```csharp
var client = new RestClient("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter");
...
```http
PUT /instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter HTTP/1.1
...
```java
HttpResponse<String> response = Unirest.put("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter")
...
```js
fetch("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter", {
  "method": "PUT",
  "headers": {
    "client-token": "{{security-token}}"
  },
...
xhr.open("PUT", "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter");
...
var options = {
  ...
  "port": null,
  "path": "/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter",
  "headers": {
    "client-token": "{{security-token}}"
  }
};
...
var req = unirest("PUT", "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter");
...
NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter"]
...
let uri = Uri.of_string "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter" in
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter",
...
$request->setUrl('https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter');
...
$request->setRequestUrl('https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter');
...
$response = Invoke-WebRequest -Uri 'https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter' -Method PUT -Headers $headers -ContentType 'undefined' -Body '{"id": "ID do Canal"}'
```
...
$headers.Add("content-type", "application/json")
$headers.Add("client-token", "{{security-token}}")
$response = Invoke-RestMethod -Uri 'https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter' -Method PUT -Headers $headers -ContentType 'undefined' -Body '{"id": "ID do Canal"}'
```
...
conn.request("PUT", "/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter", payload, headers)
...
url = URI("https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter")
...
```bash
echo '{"id": "ID do Canal"}' |  \
  http PUT https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter \
  client-token:'{{security-token}}'
```
...
let request = NSMutableURLRequest(url: NSURL(string: "https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/follow-newsletter")! as URL,
