...
Promote one or more participants of a group to admin through the Z-API.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituation}

This method allows promoting participants of a group to admin. Admins have special permissions, such as adding /remover participants, changing group settings and promoting other admins.

:::caution Caution
On November 4, 2021, WhatsApp changed the format of creating new groups:
- **Before**: `"phone": "5511999999999-1623281429"`
- **Now**: `"phone": "120363019502650977-group"`

Make sure to use the correct format of the `groupId` based on the group creation date.
:::
...
| `phones` | array string | Array with the number(s) of participant(s) to be promoted |
...
function validatePhones(phones) {
  if (!Array.isArray(phones) || phones.length === 0) {
    throw new Error('Array de telefones é obrigatório e não pode estar vazio');
  }
  for (const phone of phones) {
    if (!phone || typeof phone !== 'string' || phone.trim().length === 0) {
      throw new Error('Todos os telefones devem ser strings não vazias');
    }
  }
  return phones.map(p => p.trim());
}

// Dados da requisição com validação
const groupId = validateGroupId('120363019502650977-group');
const phones = validatePhones(['5544999999999', '5544888888888']);

// Promover admin com tratamento seguro de erros
async function addAdmin() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/add-admin`;
    ...
    if (result.value) {
      console.log('Administradores promovidos com sucesso');
    } else {
      console.error('Erro ao promover administradores');
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao promover administradores:', error.message);
    throw error;
  }
}
...
const phones: string[] = validatePhones(['5544999999999', '5544888888888']);

// Função para promover admin
async function addAdmin(): Promise<AddAdminResponse> {
...
def validate_phones(phones: List[str]) -> List[str]:
    ...
    if not phones or len(phones) == 0:
        raise ValueError('Array de telefones é obrigatório e não pode estar vazio')
    return [p.strip() for p in phones if p and p.strip()]

# Dados da requisição com validação
group_id = validate_group_id('120363019502650977-group')
phones = validate_phones(['5544999999999', '5544888888888'])

# URL do endpoint (sempre HTTPS)
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/add-admin"
...
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.get('value'):
        print('Administradores promovidos com sucesso')
    else:
        print('Erro ao promover administradores')
...
# Promover admin via cURL
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/add-admin" \
...
// Promover admin
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/add-admin`);
...
// Rota para promover admin
app.post('/add-admin', async (req, res) => {
  try {
    // Dados da requisição com validação
    const rawGroupId = req.body.groupId || '120363019502650977-group';
    const rawPhones = req.body.phones || ['5544999999999', '5544888888888'];
...
// Rota para promover admin
router.post('/add-admin', async (ctx) => {
  try {
    // Dados da requisição com validação
    const rawGroupId = ctx.request.body.groupId || '120363019502650977-group';
    const rawPhones = ctx.request.body.phones || ['5544999999999', '5544888888888'];
    const groupId = validateGroupId(rawGroupId);
    const phones = validatePhones(rawPhones);
...
app.on('error', (err, ctx) => {
  console.error('Erro ao promover administradores:', err.message);
});
...
public class AddAdmin {
    ...
    public static void main(String[] args) {
        try {
            ...
            String[] phones = {"5544999999999", "5544888888888"};

            // ⚠️ SEGURANÇA: Sempre use HTTPS
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/add-admin",
            ...
            jsonBody.put("phones", phonesArray);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Administradores promovidos com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class AddAdmin
{
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/add-admin";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Administradores promovidos com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    ...
    phones := []string{"5544999999999", "5544888888888"}

    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/add-admin",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Administradores promovidos com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        if ($result['value'] ?? false) {
            echo "Administradores promovidos com sucesso\n";
        } else {
            echo "Erro ao promover administradores\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/add-admin")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['value']
      puts "Administradores promovidos com sucesso"
    else
      puts "Erro ao promover administradores"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
let phones = ["5544999999999", "5544888888888"]

// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/add-admin") else {
...
try {
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/add-admin"
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if ($response.value) {
        Write-Host "Administradores promovidos com sucesso"
    } else {
        Write-Host "Erro ao promover administradores"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/add-admin HTTP/1.1
Host: api.z-api.io
Client-Token: SEU_CLIENT_TOKEN
Content-Type: application/json
...
int main() {
    try {
        ...
        std::vector<std::string> phones = {"5544999999999", "5544888888888"};

        // Body da requisição
        std::ostringstream jsonBody;
        jsonBody << "{\"groupId\":\"" << groupId << "\",\"phones\":[";
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/add-admin";
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Administradores promovidos com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    const char* phones[] = {"5544999999999", "5544888888888"};
    int phonesCount = 2;
    ...
    // Body da requisição
    char jsonBody[1024];
    snprintf(jsonBody, sizeof(jsonBody), "{\"groupId\":\"%s\",\"phones\":[", cleanedGroupId);
    ...
    for (int i = 0; i < phonesCount; i++) {
        ...
        len += snprintf(jsonBody + len, sizeof(jsonBody) - len, "\"%s\"", phones[i]);
    }
    snprintf(jsonBody + len, sizeof(jsonBody) - len, "]}");

    CURL* curl = curl_easy_init();
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/add-admin",
                 instanceId, instanceToken);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Administradores promovidos com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
|-------|------|------------|
| `value` | boolean | `true` if promotion was successful, `false` in case of failure |
...
| `400` | Invalid parameters | Check if the `groupId` and array `phones` are correct |
| `403` | No permission | Only admins can promote other admins |

---

## <Icon name="Info" size="md" /> Notes {#notes}

- **Permission**: Only group admins can promote other participants
- **Multiple participants**: You can promote multiple participants at once by sending multiple numbers in the `phones` array
- **Admin permissions**: Admins can add /remover participants, change settings and promote other admins
- **Group ID format**: Use the old format for groups created before November 4, 2021, or the new format for groups created after that date