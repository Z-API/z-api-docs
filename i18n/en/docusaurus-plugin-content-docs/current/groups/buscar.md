...
Search WhatsApp groups through the Z-API API. Use this endpoint to find groups by name or other criteria.
...
| --------- | ------- | --------------------------------------------------------------------------------- |
| page      | integer | Used for pagination; you need to specify here the page of groups you want to search |
| pageSize  | integer | Specifies the size of the group return per page                                   |

Example URL with attributes:
...
| muteEndTime     | string  | Timestamp with the date and time when notifications will be reactivated (-1 is forever)                               |
...
| isGroup         | boolean | true or false indicates if it's a group or not                                                                        |
...
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check the `query` |
| 401 | Invalid token | Check the header `Client-Token` |
| 404 | Chat not found | Check if the `query` is correct |
...
// Buscar grupos com tratamento seguro de erros
async function searchGroups() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/groups?page=${page}&pageSize=${pageSize}`;
    ...
    if (result.value) {
      console.log('Grupos encontrados com sucesso');
    } else {
...
async function searchGroups(): Promise<SearchGroupsResponse> {
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/groups?page={page}&pageSize={page_size}"
...
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.get('value'):
        print('Grupos encontrados com sucesso')
    else:
...
# Buscar grupos via cURL
curl -X GET \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/groups?page=${PAGE}&pageSize=${PAGE_SIZE}" \
...
// Buscar grupos
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/groups`);
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/groups?page=%d&pageSize=%d",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Grupos encontrados com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class SearchGroups
{
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/groups?page={page}&pageSize={pageSize}";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Grupos encontrados com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/groups?page=%d&pageSize=%d",
        instanceId, instanceToken, page, pageSize)
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Grupos encontrados com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        if ($result['value'] ?? false) {
            echo "Grupos encontrados com sucesso\n";
        } else {
            echo "Erro ao buscar grupos\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/groups?page=#{page}&pageSize=#{page_size}")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['value']
      puts "Grupos encontrados com sucesso"
    else
      puts "Erro ao buscar grupos"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
var urlComponents = URLComponents(string: "https://api.z-api.io/instances/\(instanceId)/token/\(instanceToken)/groups")!
...
try {
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/groups?page=$page&pageSize=$pageSize"
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if ($response.value) {
        Write-Host "Grupos encontrados com sucesso"
    } else {
        Write-Host "Erro ao buscar grupos"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
GET https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/groups?page=1&pageSize=10 HTTP/1.1
Host: api.z-api.io
...
int main() {
    try {
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/groups?page=" + std::to_string(page) + "&pageSize=" + std::to_string(pageSize);
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Grupos encontrados com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/groups?page=%d&pageSize=%d",
                 instanceId, instanceToken, page, pageSize);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Grupos encontrados com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
- Returns only groups you are part of
- The search is case-insensitive
- Groups are returned in order of relevance