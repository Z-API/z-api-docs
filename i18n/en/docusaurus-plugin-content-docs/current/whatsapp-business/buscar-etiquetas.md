...
# <Icon name="Search" size="lg" /> Search Tags

Search all tags registered in your WhatsApp Business account through the Z-API.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituacao}

This method returns all tags (tags) registered in your WhatsApp Business account. Tags are used to organize and categorize conversations.

:::important Important
This method is available only for devices connected to the **Multi-Devices** version of WhatsApp.
:::
...
// Buscar etiquetas com tratamento seguro de erros
async function getTags() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/tags`;
    ...
    if (Array.isArray(result)) {
      console.log(`Total de etiquetas: ${result.length}`);
    }
    return result;
  } catch (error) {
...
getTags()
  .then((tags) => console.log(`Total de etiquetas: ${tags.length}`))
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/tags"
...
try:
    ...
    if isinstance(result, list):
        print(f'Total de etiquetas: {len(result)}')
...
# Buscar etiquetas via cURL
curl -X GET \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/tags" \
...
// Buscar etiquetas
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/tags`);
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/tags",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Etiquetas obtidas com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class GetTags
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        {
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/tags";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Etiquetas obtidas com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/tags",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Total de etiquetas: %d\n", len(result))
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        if (is_array($result)) {
            echo "Total de etiquetas: " . count($result) . "\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/tags")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    if result.is_a?(Array)
      puts "Total de etiquetas: #{result.length}"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/tags") else {
    ...
    if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
        if let data = data {
            do {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                if let tags = result {
                    print("Total de etiquetas: \(tags.count)")
                }
            } catch {
...
try {
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/tags"
    ...
    if ($response -is [Array]) {
        Write-Host "Total de etiquetas: $($response.Count)"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
GET https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/tags HTTP/1.1
Host: api.z-api.io
...
int main() {
    ...
    if (curl) {
        std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/tags";
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            std::cout << "Etiquetas obtidas com sucesso" << std::endl;
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/tags",
                 instanceId, instanceToken);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Etiquetas obtidas com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
| `color` | number | Tag color identifier (0-5) |
...
:::tip Tip
To get the full list of available colors and their hexadecimal values, see [Tag Colors](/docs/whatsapp-business/cores-etiquetas).
:::
...
| `400` | Multi-Devices not enabled | Make sure the instance is connected to the Multi-Devices version |

---

## <Icon name="Info" size="md" /> Notes {#observacoes}

- **Multi-Devices required**: This method requires that the instance is connected to the Multi-Devices version of WhatsApp
- **Tags**: Tags are used to organize conversations and facilitate searching
- **Colors**: Each tag can have a color associated for better visualization
- **Manage tags**: You can [create](/docs/whatsapp-business/criar-etiqueta), [edit](/docs/whatsapp-business/editar-etiqueta) or [delete](/docs/whatsapp-business/deletar-etiqueta) tags
- **Assign tags**: Use [Assign Tags](/docs/whatsapp-business/atribuir-etiquetas) to associate tags with conversations