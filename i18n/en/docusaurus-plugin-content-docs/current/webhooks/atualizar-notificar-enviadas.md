...
sidebar_position: 8
title: Notify Messages Sent by Me
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# <Icon name="Bell" size="lg" /> Notify Sent Messages by Me

Configure to receive notifications for messages you sent through the "When a message is received" webhook. Useful for synchronization, logs, and auditing.

---

:::caution Caution

To make this feature work, you must have configured a webhook for the [When a message is received](/docs/webhooks/ao-receber) event.
...
- **Synchronization**: Synchronize sent messages across multiple systems
- **Logs and Auditing**: Maintain complete logs of all messages, including those you sent
- **Tracking**: Track automatically sent messages
- **Backup**: Maintain a backup of all messages in a centralized system

---

## For No-Code Users

If you use tools like n8n or Make, enable this option to receive all messages (including the ones you sent) via the same webhook. This allows:

- Processing all messages in one flow
- Maintaining a complete history of conversations
...
|----------|------|-----------|
| `notifySentByMe` | boolean | `true` to activate notifications for messages you sent, `false` to deactivate |

### <Icon name="Info" size="sm" /> Conceptualization {#conceituacao}

By default, the "When a message is received" webhook only notifies about messages received from other contacts. By activating `notifySentByMe`, you will also receive notifications for messages you sent through the API.

**Behavior**:
- **`notifySentByMe: false`** (default): Only messages received from other contacts
- **`notifySentByMe: true`**: All messages, including those you sent

**Important**:
- Requires the "When a message is received" webhook to be configured
- Messages you send will appear in the webhook with complete information
- Useful for maintaining synchronization and complete logs
...
- ⚠️ **Validation:** Validate that `notifySentByMe` is a boolean (true or false) before sending
...
| Attribute | Type | Description |
|----------|------|-----------|
| `value` | boolean | Confirms the configured value (`true` or `false`)
...
| Code | Description |
|--------|-----------|
| `405` | Incorrect HTTP method. Verify that you are using `PUT`
...
When `notifySentByMe` is activated, messages sent by you will also be notified through the webhook [Ao receber mensagem](/docs/webhooks/ao-receber).

Messages sent by you will have the same information in the webhook as received messages, allowing uniform processing.

---

## <Icon name="Lightbulb" size="md" /> Tips {#dicas}

- **Required Webhook**: Ensure you have configured the "Ao receber mensagem" webhook before activating this option
- **Synchronization**: Use this option to maintain complete synchronization between systems
- **Logs**: Ideal for systems that need to keep complete logs of all messages
- **Performance**: Consider the volume of messages when activating, as it will increase the number of received webhooks
- **Filtering**: In your system, you can filter messages sent by you using fields like `fromMe` or similar in the webhook payload

---

## Next Steps

- [Ao Receber Mensagem](/docs/webhooks/ao-receber) - Configure the main webhook
- [Atualizar Todos os Webhooks](/docs/webhooks/atualizar-todos) - Configure all webhooks at once
- [Webhooks - Introdução](/docs/webhooks/introducao) - Understand how webhooks work better