...
# <Icon name="Activity" size="lg" /> Chat Status (Presence)

Receive real-time notifications about the contact's presence status in the chat. Know when the contact is online, typing, recording audio, or away from the chat.
...
- **Typing Indicators**: Show "typing..." when the contact is writing
- **Online Status**: Detect when the contact is available in the chat
- **Recording Audio**: Know when the contact is recording an audio message
- **Real-Time Experience**: Improve UX by showing real-time status

---

## For No-Code Users

Configure this webhook in your automation tool (n8n, Make, etc.) to receive notifications about the contact's status in the chat. Use this information to:

- Show visual activity indicators
- Prioritize service for active contacts
- Create automatic responses based on presence

---

## For Developers

### <Icon name="Link" size="sm" /> Configure Webhook {#configurar}
...
| `Client-Token` | [SECURITY TOKEN](../security/token-seguranca) |

**Request Body:**

```json
{
 "value": "https://address-of-your-system.com.br/instancia/{instanceId}/presence"
}
```

#### Via Admin Panel

Access the Z-API admin panel and configure the Chat Presence webhook in the instance settings section.

---

## <Icon name="Info" size="md" /> Concept {#conceituacao}

The Chat Presence webhook notifies about changes in the contact's presence status in the chat. You will receive events when:

- The contact enters or leaves the chat
- The contact starts or stops typing
- The contact starts or stops recording audio

**Available Statuses**:
- `UNAVAILABLE`: Contact away from chat
- `AVAILABLE`: Contact inside chat
- `COMPOSING`: Contact is typing
- `RECORDING`: Contact is recording audio
- `PAUSED`: Contact stopped typing/recording (only with multi-devices)

---

## <Icon name="Webhook" size="md" /> Webhook Returns {#retornos}

### Payload Structure

| Attribute | Type | Description |
|----------|------|-------------|
| `type` | string | Event type. Will always be `"PresenceChatCallback"` |
| `phone` | string | Contact's phone number |
| `status` | string | Current chat status (see values below) |
| `lastSeen` | timestamp \| null | Timestamp of the last time the contact was present |
| `instanceId` | string | ID of the instance that received the event |

### Possible Statuses

| Status | Description |
|--------|-------------|
| `UNAVAILABLE` | Contact is away from chat |
| `AVAILABLE` | Contact is inside chat |
| `COMPOSING` | Contact is typing a message |
| `RECORDING` | Contact is recording an audio message |
| `PAUSED` | Contact stopped typing or recording (only multi-devices) |

---

## <Icon name="Code" size="md" /> Payload Examples {#exemplos}

### Contact Away from Chat
...
### Contact Stopped Typing (Multi-Devices)
...
After receiving `COMPOSING` or `RECORDING`, a `PAUSED` will be returned when the event stops.

The `PAUSED` status is only returned if you are using the multi-devices beta.

:::

### Contact Recording Audio (Multi-Devices)
...
The `RECORDING` status is only returned if you are using the multi-devices beta.

:::

---

## <Icon name="Code" size="md" /> Code Examples {#exemplos-codigo}
...
- **HTTPS mandatory**: Make sure your endpoint uses HTTPS
- **Multi-Devices**: Some statuses (`PAUSED`, `RECORDING`) only work with multi-devices enabled
- **Frequency**: You may receive multiple events quickly when the contact is typing
- **PAUSED Status**: Always received after `COMPOSING` or `RECORDING` when the event stops (only multi-devices)
- **Storage**: Consider storing the last status to improve user experience

---

## Next Steps

- [Update All Webhooks](/docs/webhooks/atualizar-todos) - Configure all webhooks at once
- [Webhooks - Introduction](/docs/webhooks/introducao) - Better understand how webhooks work
- [Multi-Devices](/docs/multi-devices/introducao) - Learn more about multi-devices (when available)