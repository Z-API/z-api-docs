...
This method is responsible for performing the action of reading a chat as a whole, or marking a chat as unread.
...
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check the `chatId` |
| 401 | Invalid token | Check the header `Client-Token` |
| 404 | Chat not found | Check if the `chatId` is correct |
...
function validateChatId(chatId) {
  ...
  if (!chatId.includes('@')) {
    throw new Error('ChatId deve estar no formato: número@c.us ou número@g.us');
  }
  return chatId.trim();
}

// Dados da requisição com validação
const chatId = validateChatId('5511999999999@c.us');

// Marcar chat como lido com tratamento seguro de erros
async function markChatAsRead() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/chats/${encodeURIComponent(chatId)}/read`;
    ...
    if (result.value) {
      ...
      console.error('Erro ao marcar chat como lido');
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao marcar chat como lido:', error.message);
    throw error;
  }
}
...
// Dados da requisição com validação
const chatId: string = validateChatId('5511999999999@c.us');

// Função para marcar chat como lido
async function markChatAsRead(): Promise<MarkReadResponse> {
...
def validate_chat_id(chat_id: str) -> str:
    """Valida formato do chatId."""
    if not chat_id or not chat_id.strip():
        raise ValueError('ChatId inválido')
    if '@' not in chat_id:
        raise ValueError('ChatId deve estar no formato: número@c.us ou número@g.us')
    return chat_id.strip()

# Dados da requisição com validação
chat_id = validate_chat_id('5511999999999@c.us')

# URL do endpoint (sempre HTTPS)
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/chats/{chat_id}/read"
...
try:
    ...
    if result.get('value'):
        ...
        print('Erro ao marcar chat como lido')
...
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/chats/${CHAT_ID}/read" \
...
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/chats/${encodeURIComponent(chatId)}/read`);
...
app.post('/chats/:chatId/read', async (req, res) => {
  try {
    // Dados da requisição com validação
    const rawChatId = req.params.chatId || '5511999999999@c.us';
...
// Rota para marcar chat como lido
router.post('/chats/:chatId/read', async (ctx) => {
  try {
    // Dados da requisição com validação
    const rawChatId = ctx.params.chatId || '5511999999999@c.us';
    const chatId = validateChatId(rawChatId);
...
public class MarkChatAsRead {
    ...
    private static String validateChatId(String chatId) {
        ...
        if (!chatId.contains("@")) {
            throw new IllegalArgumentException("ChatId deve estar no formato: número@c.us ou número@g.us");
        }
        return chatId.trim();
    }
    ...
    public static void main(String[] args) {
        try {
            // Dados da requisição com validação
            String chatId = validateChatId("5511999999999@c.us");

            // ⚠️ SEGURANÇA: Sempre use HTTPS
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/chats/%s/read",
                java.net.URLEncoder.encode(INSTANCE_ID, StandardCharsets.UTF_8),
                java.net.URLEncoder.encode(INSTANCE_TOKEN, StandardCharsets.UTF_8),
                java.net.URLEncoder.encode(chatId, StandardCharsets.UTF_8)
            );
...
public class MarkChatAsRead
{
    ...
    private static string ValidateChatId(string chatId)
    {
        ...
        {
            throw new ArgumentException("ChatId inválido");
        }
        if (!chatId.Contains("@"))
        {
            throw new ArgumentException("ChatId deve estar no formato: número@c.us ou número@g.us");
        }
        return chatId.Trim();
    }
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/chats/{Uri.EscapeDataString(chatId)}/read";
...
// Validação de entrada (segurança)
func validateChatId(chatId string) (string, error) {
    trimmed := strings.TrimSpace(chatId)
    if trimmed == "" {
        return "", fmt.Errorf("ChatId inválido")
    }
    if !strings.Contains(trimmed, "@") {
        return "", fmt.Errorf("ChatId deve estar no formato: número@c.us ou número@g.us")
    }
    return trimmed, nil
}
...
func main() {
    // Dados da requisição com validação
    chatId, err := validateChatId("5511999999999@c.us")
    if err != nil {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/chats/%s/read",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Chat marcado como lido com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
function validateChatId($chatId) {
    $trimmed = trim($chatId);
    if (empty($trimmed)) {
        throw new Exception('ChatId inválido');
    }
    if (strpos($trimmed, '@') === false) {
        throw new Exception('ChatId deve estar no formato: número@c.us ou número@g.us');
    }
    return $trimmed;
}
...
try {
    // Dados da requisição com validação
    $chatId = validateChatId('5511999999999@c.us');
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        if ($result['value'] ?? false) {
            ...
            echo "Erro ao marcar chat como lido\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
def validate_chat_id(chat_id)
  trimmed = chat_id.to_s.strip
  if trimmed.empty?
    raise ArgumentError, 'ChatId inválido'
  end
  unless trimmed.include?('@')
    raise ArgumentError, 'ChatId deve estar no formato: número@c.us ou número@g.us'
  end
  trimmed
end
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/chats/#{URI.encode_www_form_component(chat_id)}/read")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    if result['value']
      ...
      puts "Erro ao marcar chat como lido"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// Validação de entrada (segurança)
func validateChatId(_ chatId: String) throws -> String {
    let trimmed = chatId.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmed.isEmpty else {
        throw NSError(domain: "ValidationError", code: 400, userInfo: [NSLocalizedDescriptionKey: "ChatId inválido"])
    }
    guard trimmed.contains("@") else {
        throw NSError(domain: "ValidationError", code: 400, userInfo: [NSLocalizedDescriptionKey: "ChatId deve estar no formato: número@c.us ou número@g.us"])
    }
    return trimmed
}

// Dados da requisição com validação
let chatId = try validateChatId("5511999999999@c.us")

// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/chats/\(chatId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/read") else {
...
function Validate-ChatId {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        throw "ChatId inválido"
    }
    if ($trimmed -notmatch '@') {
        throw "ChatId deve estar no formato: número@c.us ou número@g.us"
    }
    return $trimmed
}
...
try {
    # Dados da requisição com validação
    $chatId = Validate-ChatId -ChatId "5511999999999@c.us"

    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/chats/$([System.Web.HttpUtility]::UrlEncode($chatId))/read"
    ...
    if ($response.value) {
        ...
        Write-Host "Erro ao marcar chat como lido"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/chats/5511999999999@c.us/read HTTP/1.1
Host: api.z-api.io
...
// Validação de entrada (segurança)
std::string validateChatId(const std::string& chatId) {
    std::string trimmed = chatId;
    trimmed.erase(0, trimmed.find_first_not_of(" \t\n\r"));
    trimmed.erase(trimmed.find_last_not_of(" \t\n\r") + 1);
    
    if (trimmed.empty()) {
        throw std::runtime_error("ChatId inválido");
    }
    if (trimmed.find('@') == std::string::npos) {
        throw std::runtime_error("ChatId deve estar no formato: número@c.us ou número@g.us");
    }
    return trimmed;
}
...
int main() {
    try {
        // Dados da requisição com validação
        std::string chatId = validateChatId("5511999999999@c.us");

        CURL* curl = curl_easy_init();
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/chats/" + chatId + "/read";
...
// Validação de entrada (segurança)
int validateChatId(const char* chatId, char* cleaned) {
    int len = strlen(chatId);
    int j = 0;
    
    // Remove espaços iniciais e finais
    int start = 0;
    while (start < len && (chatId[start] == ' ' || chatId[start] == '\t')) start++;
    int end = len - 1;
    while (end >= start && (chatId[end] == ' ' || chatId[end] == '\t')) end--;
...
int main() {
    char chatId[] = "5511999999999@c.us";
    char cleaned[256];
    
    if (!validateChatId(chatId, cleaned)) {
        fprintf(stderr, "ChatId inválido. Deve estar no formato: número@c.us ou número@g.us\n");
        return 1;
    }
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/chats/%s/read",
...
The `chatId` can have different formats depending on the chat type:
