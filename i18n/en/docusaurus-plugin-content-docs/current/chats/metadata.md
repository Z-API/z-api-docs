...
# <Icon name="Info" size="lg" /> Chat Metadata

Get detailed information about a chat through the Z-API.

---

## <Icon name="Info" size="md" /> Concept {#conceituacao}

This method returns all metadata information of a chat, including unread message count, last interaction timestamp, mute status, spam flag, profile photo, notes (WhatsApp Business), and other relevant information.
...
|-----------|------|-------------|
| `phone` | string | Recipient's phone number in DDI DDD NUMBER format (ex: `5511999999999`). **IMPORTANT**: Send only numbers, without formatting or mask |
...
// Obter metadata do chat com tratamento seguro de erros
async function getChatMetadata() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/chats/${encodeURIComponent(phone)}`;
    ...
    console.log('Metadata do chat obtida com sucesso:', data);
    return data;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro sem expor stack traces em produção
    console.error('Erro ao obter metadata do chat:', error.message);
    throw error;
  }
}
...
// Função para obter metadata do chat
async function getChatMetadata(): Promise<ChatMetadata> {
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/chats/{phone}"
...
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    print(f"Metadata do chat obtida com sucesso: {result}")
...
# Obter metadata do chat via cURL
curl -X GET \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/chats/${PHONE}" \
...
// Obter metadata do chat
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/chats/${encodeURIComponent(phone)}`);
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      ...
      // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
      console.log('Metadata do chat obtida. Phone:', result.phone);
    } else {
      // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// Rota para obter metadata do chat
app.get('/chat-metadata/:phone', async (req, res) => {
  try {
    // Dados da requisição com validação
...
// Rota para obter metadata do chat
router.get('/chat-metadata/:phone', async (ctx) => {
  try {
    // Dados da requisição com validação
...
app.on('error', (err, ctx) => {
  console.error('Erro ao obter metadata do chat:', err.message);
});
        ...
        try {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Metadata do chat obtida. Response: " + response.toString());
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class GetChatMetadata
{
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/chats/{Uri.EscapeDataString(phone)}";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine($"Metadata do chat obtida. Response: {result}");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Metadata do chat obtida com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        echo "Metadata do chat obtida com sucesso\n";
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/chats/#{URI.encode_www_form_component(phone)}")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    puts "Metadata do chat obtida com sucesso"
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/chats/\(phone.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")") else {
    ...
    if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
        if let data = data {
            do {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                print("Metadata do chat obtida com sucesso")
            } catch {
...
try {
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/chats/$([System.Web.HttpUtility]::UrlEncode($phone))"
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    Write-Host "Metadata do chat obtida com sucesso"
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
GET https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/chats/5511999999999 HTTP/1.1
Host: api.z-api.io
...
int main() {
    try {
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/chats/" + phone;
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Metadata do chat obtida com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Metadata do chat obtida com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
| `unread` | string | Unread message count in the chat |
| `lastMessageTime` | string | Timestamp with date and time of last interaction with the chat |
| `isMuted` | string | `"0"` or `"1"` - indicates if you muted this chat |
| `isMarkedSpam` | boolean | `true` or `false` - indicates if you marked this chat as spam |
| `profileThumbnail` | string | URL of the chat photo. **WhatsApp deletes it after 48h** |
| `isGroupAnnouncement` | boolean | `true` or `false` - indicates if the chat is a warning group |
| `isGroup` | boolean | `true` or `false` - indicates if the chat is a group |
| `notes` | object | Notes assigned to the chat (only WhatsApp Business) |
| `notes.id` | string | Note ID |
| `notes.content` | string | Note text |
| `notes.createdAt` | number | Timestamp of note creation |
| `notes.lastUpdateAt` | number | Timestamp of last note update |
| `messagesUnread` | integer | **Deprecated** - use `unread` |
...
| `400` | Invalid parameters | Check if the `phone` is in correct format (only numbers) |
| `404` | Chat not found | Check if the number exists and has conversation history |

---

## <Icon name="Info" size="md" /> Notes {#observacoes}

- **Phone format**: Send only numbers, without formatting or mask (ex: `5511999999999`)
- **Profile photo**: The URL `profileThumbnail` expires after 48 hours
- **Notes**: The field `notes` is only available for WhatsApp Business accounts
- **Groups**: Use `isGroup` to identify if the chat is a group or individual conversation
- **Warning groups**: `isGroupAnnouncement` indicates if it's a group where only admins can send messages
- **Unread messages**: Use the field `unread` (string) instead of `messagesUnread` (deprecated)
- **Spam**: Use `isMarkedSpam` to check if the chat was marked as spam
- **Muted**: Use `isMuted` to check if the chat is muted (`"1"` = muted, `"0"` = not muted)

---

## <Icon name="Link" size="md" /> Related Methods {#relacionados}

- [List Chats](/docs/chats/pegar) - List all chats
- [Read Chat](/docs/chats/ler) - Mark messages as read
- [Archive Chat](/docs/chats/arquivar) - Archive or unarchive a chat
- [Pin Chat](/docs/chats/fixar) - Pin or unpin a chat
- [Mute Chat](/docs/chats/mutar) - Mute or unmute a chat
- [Clear Chat](/docs/chats/limpar) - Clear message history
