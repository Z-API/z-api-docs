...
# <Icon name="ShoppingCart" size="lg" /> Send Order

Send complete orders with products from your catalog or custom products. Ideal for e-commerce, billing, and order approval on WhatsApp Business.
...
- **E-commerce**: Send orders with catalog products
- **Billing**: Send invoices to customers
- **Order Approval**: Send orders for customer approval
- **Custom Products**: Create orders with non-cataloged products

![Example of order - Screen 1](/img/send-order1.jpeg)

![Example of order - Screen 2](/img/send-order2.jpeg)
...
1. **`phone`**: Recipient number (format: `5511999999999`)
2. **`order`**: Object with order information (currency, products, discounts, etc.)
3. **`paymentSettings`** (optional): Payment settings (PIX, card)
...
|----------|------|-----------|
| `phone` | string | Recipient phone number in format DDI DDD NUMBER (ex: `5511999999999`). **IMPORTANT**: Send only numbers, without formatting or mask |
| `order` | object | Order information (see structure below) |
| `paymentSettings` | object | Payment settings (see structure below) |

#### Structure of Object `order`

| Attribute | Type | Required | Description |
|----------|------|---------|-----------|
| `currency` | string | Yes | Currency code (ex: `BRL`, `USD`) |
| `products` | array | Yes | Array of order products (see structure below) |

**Optional attributes of `order`**
...
#### Structure of Object `products` (inside `order`)
...
| `productId` | string | No | Catalog product ID. If not provided, it will be a custom product |

#### Structure of Object `paymentSettings`

| Attribute | Type | Required | Description |
|----------|------|---------|-----------|
| `pix` | object | No | Pix key information (see structure below) |
| `card` | object | No | Card payment configuration (see structure below) |

#### Structure of Object `pix` (inside `paymentSettings`)

| Attribute | Type | Required | Description |
|----------|------|---------|-----------|
| `key` | string | Yes | Pix key |
| `keyType` | string | Yes | Key type: `cpf`, `cnpj`, `phone`, `email`, `randomKey` |
| `name` | string | Yes | Key name |

#### Structure of Object `card` (inside `paymentSettings`)

| Attribute | Type | Required | Description |
|----------|------|---------|-----------|
| `enabled` | boolean | Yes | Enable card payment (requires configuration in the WhatsApp account via mobile) |

### <Icon name="Info" size="sm" /> Conceptualization {#conceituacao}

This method allows sending complete orders with catalog or custom products. The message sent is the same as when you click on "Accept order" or "Send invoice" in the customer's order.

**Important**:
- The webhook will return information about the order, including `referenceId` and `orderRequestId`
- These IDs are necessary for updating the order status and payment
- Products without `productId` are considered "customized" and receive a temporary ID
...
| `messageId` | string | Message ID in WhatsApp (use this ID for order updates) |
| `id` | string | Compatibility ID with Zapier (same value as `messageId`) |
...
| `415` | Content-Type missing. Add `Content-Type: application/json` to header |

---

## <Icon name="Webhook" size="md" /> Related Webhook {#webhook}

When an order is sent, the [Receive message](/docs/webhooks/ao-receber) webhook will be triggered with important information:

- **`referenceId`**: Order reference ID (use for updates)
- **`orderRequestId`**: Request order ID (use for updates)
- **`productId`**: Product IDs (including custom products with prefix `custom-item-`)
- **`isCustomItem`**: Indicates if the product is custom

For more details, see the [Receive message](/docs/webhooks/ao-receber#example-of-return-for-order-sending) webhook documentation.
...
When sending a product without the attribute `productId`, it will be considered a "customized" product. WhatsApp will assign a temporary ID with the prefix `custom-item-`, which will be returned in the webhook and should be used for order updates.

:::

- **Save IDs**: Save the `messageId`, `referenceId` and `orderRequestId` returned in the webhook for future updates
- **Catalog Products**: Use `productId` for products already registered in your catalog
- **Credit Card Payment**: Requires prior configuration on the WhatsApp account via mobile phone
- **Currency**: Use ISO currency codes (ex: `BRL`, `USD`, `EUR`)

---

## <Icon name="Rocket" size="md" /> Next Steps

- [Update Order Status](/docs/messages/atualizacao-status-pedido) - Update the order status (pending, processing, shipped, completed, canceled)
- [Update Order Payment](/docs/messages/atualizacao-pagamento-pedido) - Update the payment status (pending, paid)
- [Webhooks](/docs/webhooks/introducao) - Configure webhooks to receive notifications about orders