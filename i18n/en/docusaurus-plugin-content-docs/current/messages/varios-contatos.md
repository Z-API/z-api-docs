...
# <Icon name="Users" size="lg" /> Send Multiple Contacts

Send multiple contacts to a recipient using the Z-API's API. This method allows you to send multiple contacts; you don't need them in your contacts, just fill in the attributes of the method with contact information and send.

![Example of sending to multiple contacts](/img/send-message-contacts.jpeg)
...
|-----------|--------|-------------|--------------------------------------------------|
| `phone` | string | Yes | Recipient's phone number (or group ID for group sends) in the format DDI DDD NUMBER. **IMPORTANT:** Send only numbers, without formatting or mask |
| `contacts` | array | Yes | Array of contacts to be sent |
...
| `phones` | array | Yes | Phone numbers of the contact (array of strings) |
| `businessDescription` | string | No | Brief description about the contact (optional) |

#### Optional Parameters

| Field | Type | Required | Description |
|-----------|--------|-------------|--------------------------------------------------|
| `messageId` | string | No | Attribute used to reply to a chat message. Just add the `messageId` of the message you want to reply in this attribute |
| `delayMessage` | number | No | This attribute adds a delay to the message. You can decide between a range of 1~15 sec, meaning how many seconds it will wait before sending the next message. (Ex "delayMessage": 5, ). The default delay if not informed is 1~3 sec |
...
- **`phone`**: The recipient's phone number to which you want to send the contacts. Use the full format: DDI + DDD + Number (ex: `5544999999999`). **Important:** Use only numbers, without formatting or mask. For groups, use the group ID.

- **`contacts`**: An array of contact objects. Each contact should have:
  - **`name`**: Contact name (required)
  - **`phones`**: Array of phone numbers for the contact (required)
  - **`businessDescription`**: Brief description about the contact (optional)

### Optional Fields

- **`messageId`**: (Optional) ID of the message you want to reply. Use this field if you want to send the contacts as a response to a specific message.

- **`delayMessage`**: (Optional) Time in seconds before sending the message. Use a value between 1 and 15 seconds. If not informed, the default delay is 1~3 seconds.
...
- **Contact Structure**: Each contact in the array `contacts` must have at least `name` and `phones`. The field `businessDescription` is optional.
- **Multiple Phone Numbers per Contact**: A contact can have multiple phone numbers in the array `phones`.
- **No need to be saved in WhatsApp**: You don't need to have the contacts saved in WhatsApp. Just fill in the information in the object.
- **Response**: The response will be an object with `zaapId`, `messageId` and `id` (for compatibility with Zapier).

**Common Use Cases:**

- **Sharing Contacts**: Send contact information of multiple people to a recipient
- **Team Directory**: Send list of contacts for team members
- **Referrals**: Share contacts of professionals or services
- **Event Schedule**: Send contacts of organizers or participants
...
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check `phone` and `contacts` (must be array with valid objects) |
...
- **Maximum Recipients**: Recommended up to 100 recipients per request for better performance
- **Rate limiting**: Respect the API rate limits to avoid blocks
- **Validation**: Validate numbers before sending to improve delivery rates
- **Personalization**: Consider using personalized messages for each recipient when possible

:::warning Important

Do not use this endpoint for spam or unsolicited message sends. Respect WhatsApp's terms of service and use the API responsibly.

:::

## <Icon name="Info" size="md" /> Important Notes {#notas-importantes}

- All numbers should be in international format (DDI + DDD + number)
...
- [Send Simple Text](/docs/messages/texto-simples) - Send individual messages
- [Forward Message](/docs/messages/reencaminhar) - Forward received messages
- [Validate Numbers](/docs/contacts/numero-whatsapp) - Validate numbers before sending