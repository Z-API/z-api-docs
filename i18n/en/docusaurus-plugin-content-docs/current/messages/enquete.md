...
# <Icon name="BarChart" size="lg" /> Send Poll

Send interactive polls to your contacts or groups via WhatsApp. Polls allow recipients to vote on one or multiple options, making it easy to collect opinions and feedback.

---

## <Icon name="Lightbulb" size="md" /> Common Use Cases

- **Satisfaction Surveys**: Collect feedback on products or services
- **Group Voting**: Decide between options in work groups or communities
- **Opinion Polls**: Discover customer preferences
- **Data Collection**: Obtain structured information interactively
...
2. **`message`**: Poll question text
3. **`poll`**: Array with poll options (minimum 2 options)
4. **`pollMaxOptions`** (optional): Maximum number of votes per person (default: multiple choices)
...
### <Icon name="Settings" size="sm" /> Attributes {#atributos}

#### Required

| Attribute | Type | Description |
|----------|------|-----------|
| `phone` | string | Recipient phone number in the format DDI DDD NUMBER (ex: `5511999999999`). **IMPORTANT**: Send only numbers, without formatting or mask. For groups, use the group ID. |
| `message` | string | Poll question text |
| `poll` | PollItem[] | List of poll options (minimum 2 options) |

#### Optional

| Attribute | Type | Description |
|----------|------|-----------|
| `delayMessage` | number | Delay in seconds (1-15). Default delay is 1-3 seconds if not specified |
| `pollMaxOptions` | number | Maximum number of votes per person. If not specified, allows multiple choices. Use `1` for single choice |
...
Polls are an interactive way to collect opinions and feedback. You can create polls with multiple choices (default) or single choice (defining `pollMaxOptions: 1`).

**Important:**
- The recipient can vote on a received poll
- You can vote on polls you have sent yourself
- Use the `messageId` returned to allow voting in the poll
...
| `messageId` | string | WhatsApp message ID (use this ID to vote on the poll) |
...
When a poll is received, the webhook [On receive message](/docs/webhooks/ao-receber) will be triggered with type `poll`.

For more details on how to receive and process polls, see the webhook documentation [On receive message](/docs/webhooks/ao-receber#example-of-poll-return).

---

## <Icon name="Lightbulb" size="md" /> Tips {#dicas}

- **Store the `messageId`**: You will need the `messageId` returned to allow users to vote on polls using the [Vote in Poll](/docs/messages/voto-enquete) endpoint
- **Minimum options**: A poll should have at least 2 options
- **Single choice vs multiple choices**: Use `pollMaxOptions: 1` for single choice polls
- **Polls in groups**: You can send polls to groups using the group ID in the field `phone`

---

## <Icon name="Rocket" size="md" /> Next Steps

- [Vote in a Poll](/docs/messages/voto-enquete) - Learn how to allow users to vote on polls
- [Webhooks](/docs/webhooks/introducao) - Configure webhooks to receive notifications about polls