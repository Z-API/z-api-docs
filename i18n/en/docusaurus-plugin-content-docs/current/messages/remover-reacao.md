...
# <Icon name="XCircle" size="lg" /> Remove Reaction

Remove an emoji reaction from an existing message using the Z-API's API. Use this endpoint to remove a reaction that you previously added.

In this method, you will remove the reaction from messages sent or received. You need to provide the chat phone number and the `messageId` of the message.

![Example of removing reaction](/video/remove-reaction.gif)
...
| `messageId` | string | Yes | ID of the message from which the reaction will be removed |

#### Optional Parameters

| Field | Type | Description |
|----------|------|-----------|
| `delayMessage` | number | This attribute adds a delay to the message. You can choose between a range of 1\~15 sec, meaning how many seconds it will wait before sending the next message. (Ex "delayMessage": 5, ). The default delay if not specified is 1\~3 sec |
...
2. **`messageId`**: The ID of the message from which the reaction will be removed. This is the same `messageId` as the message that has the reaction you want to remove. You can get the `messageId` through webhooks when a message is received or sent.

### Optional Fields

3. **`delayMessage`**: Delay in seconds (1-15 seconds) before removing the reaction. If not specified, the default delay is 1-3 seconds.
...
- **Get messageId**: The `messageId` can be obtained through webhooks when a message is received or sent. Save this ID if you want to remove the reaction later.
- **Message must have reaction**: The specified `messageId` message must have a reaction for the removal to work.
- **Reactions in groups**: You can remove reactions from messages in groups using the group ID in the `phone` field.
- **Optional delay**: Use `delayMessage` if you want to add a delay before removing the reaction (useful for simulating human behavior).

**Common Use Cases:**

- **Correcting errors**: Removing an accidentally sent reaction
- **Updating feedback**: Removing an old reaction before adding a new one
- **Automated customer service**: Removing automatic reactions after a period of time
- **Managing interactions**: Managing reactions in automated conversations
...
To know when the reaction was removed or if there was an error, configure a webhook and monitor events. See more about [message received webhooks](../webhooks/ao-receber#exemplo-de-retorno-de-reação). **NOTE:** In webhook return, the "Reaction" field will be empty when the reaction is removed.

### Common Errors {#erros-comuns}

| Code | Reason | How to resolve |
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check `phone` and `messageId` |
...
To remove a reaction, you must:

1. **Use the correct endpoint** (`/send-remove-reaction`)
2. **Provide the `messageId`** of the message that has the reaction you want to remove
3. **Provide the `phone`** of the chat where the message is located (or group ID for groups)

**Important:**

- The specified `messageId` message must have a reaction for the removal to work
- To replace an existing reaction, use the [add reaction](.//reacao) endpoint with a new emoji

## <Icon name="AlertTriangle" size="md" /> Limitations {#limitacoes}

- You can only remove reactions that you yourself added
- The original message must exist and be accessible
- If the message does not have a reaction, the operation will still return success

## <Icon name="Info" size="md" /> Important Notes {#notas-importantes}

- Removing a reaction is equivalent to sending an empty string in the `reaction` field
- The reaction disappears immediately for the original message sender
- Use the `messageId` of the message that has the reaction you want to remove
- The `phone` must be the sender's phone number for the original message

## <Icon name="ArrowRight" size="md" /> Next Steps {#proximos-passos}

- [Add Reaction](/docs/messages/reacao) - Add an emoji reaction to a message
