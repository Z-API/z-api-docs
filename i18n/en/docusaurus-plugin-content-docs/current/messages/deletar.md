...
# <Icon name="Trash2" size="lg" /> Delete Message

Delete a message sent using the Z-API. The message will be removed for all participants of the conversation.

Method used to delete a message in a chat. You can delete both messages that you have sent and messages sent by contacts. To use this feature, you only need the `messageId` of the message you want to delete.

![Example of deleting a message](/img/delete-message.jpeg)
...
### Query Parameters {#parametros-de-query}

#### Required Parameters

| Parameter | Type | Required | Description |
|-----------|--------|-------------|--------------------------------------------------|
| `messageId` | string | Yes | Original message ID. In the case of a message you sent, it is the code in your response. If it was a message from a contact, you will receive this `messageId` via your receive webhook |
| `phone` | string | Yes | Recipient's phone number (or group ID for cases of sending to groups) in the format DDI DDD NUMBER. **IMPORTANT:** Send only numbers, without formatting or mask |
| `owner` | boolean | Yes | Indicate `true` if you sent the message or `false` for cases where it was a received message |
...
1. **`messageId`**: The ID of the message you want to delete. This is the `messageId` that you received through webhooks or which you sent previously (obtained in the API response).

2. **`phone`**: The phone number of the chat where the message is located. Use the full format: DDI + DDD + Number (ex: `5511999999999`). **Important:** Send only numbers, without formatting or mask. For groups, use the group ID.

3. **`owner`**: Indicate if you are the owner of the message. Use `true` if you sent the message, or `false` if it was a received message from a contact.

### Practical Example for No-Code

**Delete message that you sent:**
...
- **Get messageId**: The `messageId` can be obtained through webhooks when a message is received or sent. Save this ID if you want to delete the message later.
- **owner Parameter**: It's important to correctly inform whether you are the owner of the message (`true`) or not (`false`). This determines if you can delete the message.
- **Group messages**: You can delete messages in groups using the group ID in the `phone` field.
- **Status 204**: The response is a status `204 No Content`, indicating that the operation was successful. There is no content in the body of the response.

**Common Use Cases:**

- **Error correction**: Delete messages sent by mistake
- **Privacy**: Remove sensitive or confidential messages
- **Content cleanup automation**: Automatically delete old messages
- **Content management**: Manage messages in automated conversations

## <Icon name="CheckCircle" size="md" /> Responses {#respostas}

### 204 No Content {#204-no-content}

The response is a status `204 No Content`, indicating that the message was deleted successfully. The body of the response is empty.
...
- There are no fields in the response, only the HTTP status indicates success
...
| 403 | No permission | You can only delete messages that you sent |
| 404 | Message not found | Check if the `messageId` exists and is valid |
...
- Very old messages may not be deletable
- Deletion may take a few seconds to process

:::warning Important

Deletion is permanent and cannot be undone. Make sure you really want to delete the message before performing this operation.

:::

## <Icon name="Info" size="md" /> Important Notes {#notas-importantes}

- The message will be removed for all participants of the conversation
- Use the `messageId` returned when you sent the message
- Deleted messages cannot be recovered
- Some types of messages may have deletion restrictions
- Deletion may not be instantaneous and may take a few seconds to process

## <Icon name="ArrowRight" size="md" /> Next Steps {#proximos-passos}

- [Mark as read](/docs/messages/ler) - Mark messages as read
