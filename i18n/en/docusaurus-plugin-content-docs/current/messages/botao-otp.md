...
# <Icon name="Key" size="lg" /> OTP Button

Send messages with a button to copy one-time passwords (OTP). Ideal for authentication, verification codes, and values that need to be copied quickly.

---

## <Icon name="Lightbulb" size="md" /> Common Use Cases

- **Verification Codes**: Send two-factor authentication (2FA) verification codes
- **Access Tokens**: Share temporary tokens securely
- **Discount Codes**: Send coupons and promotional codes
- **Temporary Passwords**: Share temporary access passwords
...
![Example of OTP button](/img/SendButtonOtp.jpeg)
...
- **`phone`**: The recipient's phone number where you want to send the OTP button. Use full format: DDI + DDD + Number (ex: `5511999999999`). **Important**: Send only numbers, without formatting or mask. For groups, use the group ID.

- **`message`**: The text of the message that will be displayed along with the button. This is the explanatory text that appears before the button.

- **`code`**: The value that will be copied when the button is clicked. This is the OTP code, token or value you want to make it easy for users to copy.

### Optional Fields

- **`image`**: (Optional) URL or Base64 of the image that will accompany the button. You can include an image to make the message more visual and attractive.

- **`buttonText`**: (Optional) Text of the button. The default value is "Copy code" if not specified. You can customize this text for something more specific, like "Copy verification code" or "Copy token".
...
**Example with image and custom text:**
...
- **OTP Code**: The code can be alphanumeric and usually has between 4 to 10 characters. Use codes that are easy to type but secure.
- **Button Text**: Customize the button text to make it clear what will be copied. This improves user experience.
- **Image**: Use relevant images that reinforce the message. For example, a security image for verification codes.
- **Response**: The response will be an object with `zaapId`, `messageId` and `id` (for compatibility with Zapier). Use the `messageId` to track message status through webhooks.

**Common use cases:**

- **Two-Factor Authentication**: Send two-factor authentication (2FA) verification codes
- **Access Tokens**: Share temporary tokens securely
- **Discount Codes**: Send coupons and promotional codes
- **Temporary Passwords**: Share temporary access passwords
- **Confirmation Codes**: Send codes to confirm actions (registrations, purchases, etc.)
...
|----------|------|-----------|
| `phone` | string | Recipient's phone number in the format DDI DDD NUMBER (ex: `5511999999999`). **Important**: Send only numbers, without formatting or mask. For groups, use the group ID. |
| `message` | string | Text of the message that will be displayed |
...
| `buttonText` | string | Text of the button. The default value is "Copy code" if not specified |

### <Icon name="Info" size="sm" /> Conceptualization {#conceptualization}

The OTP button allows creating messages with an interactive button that automatically copies a value to the user's clipboard when clicked. This is especially useful for verification codes, tokens, and values that need to be copied quickly.

**Features:**

- The button copies the value automatically when clicked
- You can customize the button text
- Can include an image along with the message
- Works in individual conversations and groups

---

## <Icon name="Code" size="md" /> Code Examples {#code-examples}
...
| `id` | string | ID for compatibility with Zapier (same value as `messageId`) |
...
- The `messageId` is the primary identifier that you should use to track the message
...
To know when the message was delivered, read or if there was an error, configure a webhook and monitor events. See more about [message received webhooks](../webhooks/ao-receber#example-of-return-of-template-of-otp-button).

### Common Errors {#common-errors}

| Code | Reason | How to resolve |
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check `phone`, `message` and `code` |
...
When an OTP button message is received, the webhook [On Message Received](/docs/webhooks/ao-receber) will be triggered with the type `button`.

For more details on how to receive and process messages with buttons, see the webhook documentation [On Message Received](/docs/webhooks/ao-receber#example-of-return-of-template-of-otp-button).

---

## <Icon name="Lightbulb" size="md" /> Tips {#tips}

- **Temporary Codes**: Use OTP codes for two-factor authentication (2FA)
- **Customization**: You can customize the button text to improve user experience
- **Images**: Add an image to make the message more visual and attractive
- **Security**: OTP codes should be temporary and unique for each use
- **Button Functionality**: Consult [**Button Functionality**](/docs/tips/button-status) to understand the factors that affect functionality

---

## <Icon name="Rocket" size="md" /> Next Steps

- [Pix Button](/docs/messages/botao-pix) - Send buttons for Pix payments
- [Other types of buttons](/docs/messages/botoes) - Explore other interactive button types
