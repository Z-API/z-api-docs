...
# <Icon name="CreditCard" size="lg" /> Update Payment for Order

Update the payment status of an order that was sent previously. Use this method to notify the customer when the payment is confirmed or updated.
...
- **Payment Confirmation**: Notify when the payment was confirmed
- **Pending Payment**: Update status to pending payment
- **Gateway Integration**: Synchronize payment status with payment gateways

![Example of order payment update](/img/order-payment-update.jpeg)

---

## <Icon name="Wand2" size="md" /> For No-Code Users

To update the payment of an order, you will need the following data returned in the webhook when the order was sent:

1. **`messageId`**: Original message ID of the order
2. **`referenceId`**: Order reference ID
3. **`orderRequestId`**: Request ID of the order
4. **`orderStatus`**: Current status of the order
...
| `orderRequestId` | string | Request ID of the order (returned in webhook) |
| `orderStatus` | string | Current order status: `pending`, `processing`, `shipped`, `completed`, `canceled` (returned in webhook) |
| `paymentStatus` | string | New payment status: `pending` or `paid` |
...
| `shipping` | number | No | Shipping value |
...
All previously informed data must be sent in the request, including the additional required parameters returned in the webhook.
...
When the order payment is updated, the [Receive message](/docs/webhooks/ao-receber) webhook will be triggered with information about the update.

For more details, see the webhook documentation [Receive message](/docs/webhooks/ao-receber#example-of-order-payment-update).
...
- `productId` and `isCustomItem`: For each order product
...
- **Order status**: Use the `orderStatus` returned in webhook, do not change this value here
- **Custom message**: Use the field `message` to personalize the notification to the customer
- **Gateway integration**: Use this endpoint when receiving payment confirmation from your gateway

---

## <Icon name="Rocket" size="md" /> Next Steps

- [Update Order Status](/docs/messages/atualizacao-status-pedido) - Update the order status
- [Send Order](/docs/messages/aprovacao-pedido) - Learn to send orders
