...
id: botoes-imagem
sidebar_position: 19
title: Buttons with Image
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# <Icon name="Image" size="lg" /> Image Buttons

Send images with quick reply buttons. The user can choose an option by clicking one of the buttons, and the response will be captured via webhook.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituacao}

This method allows sending an image with quick reply buttons. When the user clicks a button, the response will be sent as a message that can be captured via webhook, allowing for interactive flows based on the user's choice.
...
:::important Important
The `message` attribute cannot be sent empty! Always include text along with the image.
:::

![Example of image buttons](/img/send-button-list-image.jpeg)
...
- **`phone`**: The recipient number to which you want to send the image with buttons. Use the full format: DDI + DDD + Number (ex: `5511999999999`). **Important:** Use only numbers, without formatting or mask. For groups, use the group ID.

- **`message`**: The text that will be displayed along with the image. **This field is required and cannot be empty!** Use this field to explain what the user should do or provide context about the image.

- **`buttonList`**: An object containing the image and buttons. Within this object, you need to configure:

  - **`image`**: The public URL or Base64 of the image that will be sent. The image must be publicly accessible or encoded in Base64.

  - **`buttons`**: An array list of buttons. Each button needs to have:
...
- **Required message**: The `message` field cannot be empty. Always include an explanatory text along with the image.
- **Image URL**: Make sure the image URL is public and accessible. Test the URL in your browser before using it in automation.
- **Base64**: If you prefer to use Base64, the format should be: `data:image/jpeg;base64,SEU_CODIGO_BASE64` (or `data:image/png;base64,` for PNG).
- **Button ID**: Use descriptive IDs (ex: `"sim"`, `"nao"`, `"opcao1"`) to facilitate identification in webhooks when the user clicks.
...
- **Satisfaction surveys**: Send product image with evaluation buttons
- **Visual confirmations**: Send confirmation image with "Confirm" and "Cancel" buttons
- **Interactive catalogs**: Send product image with "View more", "Buy", "Favorite" buttons
- **Visual support**: Send instruction image with quick reply buttons
- **Promotions**: Send promotional image with "I want it", "See details", "Share" buttons
...
When the user clicks a button, you will receive a webhook with the response. The webhook will include the `id` of the clicked button, allowing you to identify which option the user chose. See more about [received message webhooks](../webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem).
...
|----------|------|------------|
| `phone` | string | Recipient phone (or group ID for sending to groups) in the format DDI DDD NUMBER. Ex: `551199999999`. **Important**: Send only numbers, without formatting or mask |
| `message` | string | Text to be sent along with the image. **Cannot be empty** |
| `buttonList` | object | Object containing the image and buttons |

### buttonList

| Attribute | Type | Description |
|----------|------|------------|
| `image` | string | URL or Base64 of the image that will be sent |
...
async function sendButtonListWithImage(phone, message, imageUrl, buttons) {
  try {
    ...
    console.log('Botões com imagem enviados com sucesso');
    return data;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro sem expor stack traces em produção
    console.error('Erro ao enviar botões com imagem:', error.message);
    throw error;
  }
}

// Exemplo de uso
sendButtonListWithImage(
  '5511999999999',
  'Z-API é Bom ?',
  'https://avatars.githubusercontent.com/u/60630101?s=280&v=4',
  [
    { id: '1', label: 'Ótimo' },
    { id: '2', label: 'Excelente' },
  ]
...
// Função para enviar botões com imagem
async function sendButtonListWithImage(
  phone: string,
  message: string,
  imageUrl: string,
  buttons: Button[]
...
def validate_image_url(url: str) -> str:
    """Valida URL ou Base64 da imagem"""
...
def send_button_list_with_image(
    phone: str,
    message: str,
    image_url: str,
    ...
    try:
        ...
        # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        print('Botões com imagem enviados com sucesso')
        return result
...
# ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
# Enviar botões com imagem via cURL
curl -X POST \
...
// Enviar botões com imagem
function sendButtonListWithImage(phone, message, imageUrl, buttons) {
...
// Rota para enviar botões com imagem
app.post('/api/send-button-list-image', async (req, res) => {
  try {
    // ⚠️ VALIDAÇÃO
    ...
    res.status(error.response?.status || 500).json({
      success: false,
      error: 'Erro ao enviar botões com imagem',
    });
  }
});
...
// Middleware para enviar botões com imagem
app.use(async (ctx) => {
  if (ctx.path === '/api/send-button-list-image' && ctx.method === 'POST') {
    try {
      // ⚠️ VALIDAÇÃO
      const { phone, message, imageUrl, buttons } = ctx.request.body;
...
public class SendButtonListImage {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    public static void main(String[] args) {
        try {
            ...
            String imageUrl = "https://avatars.githubusercontent.com/u/60630101?s=280&v=4";
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                System.out.println("Botões com imagem enviados com sucesso");
        ...
        try
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Console.WriteLine("Botões com imagem enviados com sucesso");
...
func main() {
    ...
    imageUrl := "https://avatars.githubusercontent.com/u/60630101?s=280&v=4"
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        fmt.Println("Botões com imagem enviados com sucesso")
...
try {
    ...
    if ($error) {
        ...
        echo "Botões com imagem enviados com sucesso\n";
...
begin
  ...
  image_url = 'https://avatars.githubusercontent.com/u/60630101?s=280&v=4'
  buttons = [
    { id: '1', label: 'Ótimo' },
    { id: '2', label: 'Excelente' }
  ]
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    puts 'Botões com imagem enviados com sucesso'
    puts result.to_json
  else
...
do {
    ...
    let imageUrl = "https://avatars.githubusercontent.com/u/60630101?s=280&v=4"
...
try {
    ...
    Write-Host "Botões com imagem enviados com sucesso"
...
int main() {
    ...
    std::string imageUrl = "https://avatars.githubusercontent.com/u/60630101?s=280&v=4";
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                std::cout << "Botões com imagem enviados com sucesso" << std::endl;
...
int main() {
    ...
    char* imageUrl = "https://avatars.githubusercontent.com/u/60630101?s=280&v=4";
    ...
    snprintf(payload, sizeof(payload),
        "{\"phone\":\"%s\",\"message\":\"%s\",\"buttonList\":{\"image\":\"%s\",\"buttons\":[{\"id\":\"1\",\"label\":\"Ótimo\"},{\"id\":\"2\",\"label\":\"Excelente\"}]}}",
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                printf("Botões com imagem enviados com sucesso\n");
                printf("%s\n", responseData);
            } else {
                printf("Erro HTTP %ld\n", responseCode);
            }
...
To know when the message was delivered, read, or when a button was clicked, configure a webhook and monitor the events. See more about [received message webhooks](../webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem).

### Common Errors {#erros-comuns}

| Code | Reason | How to Solve |
|------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check if all required attributes were sent, especially if `message` is not empty, if `buttonList` contains `image` and `buttons`, and if there is at least 1 button |
| 401 | Invalid token | Check the header `Client-Token` |
| 405 | Incorrect method | Make sure you are using the method `POST` |
| 415 | Incorrect Content-Type | Add `Content-Type: application/json` in the request headers |
...
[Webhook on receiving message - Image buttons](/docs/webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem)

---

## <Icon name="Info" size="md" /> Notes {#observacoes}

- **Image**: Can be provided via public URL or Base64
- **Required message**: The `message` attribute cannot be empty
- **Button ID**: Use the `id` to identify which button was clicked in webhooks
