...
# <Icon name="Forward" size="lg" /> Forward message

Forward an existing message to a recipient using the Z-API. Use this endpoint to share messages you have received.

Simple and objective, with this method you can forward messages through the API needing only the `messageId` of the message you want to forward, and the `phone` of the chat where that `messageId` is located.
...
![Forwarding example](/img/reencaminhar.jpeg)
...
|-----------|--------|-------------|--------------------------------------------------|
| `phone` | string | Yes | Recipient's phone (or group ID for group sending) in DDI DDD NUMBER format. **IMPORTANT:** Send only numbers, without formatting or mask |
| `messageId` | string | Yes | ID of the message to be forwarded |
| `messagePhone` | string | Yes | Number of the chat where the `messageId` is located |

#### Optional Parameters

| Field | Type | Required | Description |
|-----------|--------|-------------|--------------------------------------------------|
| `delayMessage` | number | No | A delay is added to the message. You can choose a range of 1~15 sec, meaning how many seconds it will wait to send the next message. (Ex "delayMessage": 5, ). The default delay if not informed is 1~3 sec |
...
1. **`phone`**: The recipient's number where you want to forward the message. Use the full format: DDI + DDD + Number (ex: `5511999999999`). **Important:** Use only numbers, without formatting or mask. For groups, use the group ID.

2. **`messageId`**: The ID of the message you want to forward. This is the `messageId` of the message you received through webhooks or that you sent previously.

3. **`messagePhone`**: The number of the chat where the original message (`messageId`) is located. This is the number of the original sender or the group where the message was received/sent.

### Optional Field

1. **`delayMessage`**: (Optional) Waiting time in seconds before sending the message. Use a value between 1 and 15 seconds. If not informed, the default delay is 1~3 seconds.
...
- **Content sharing**: Forward interesting messages to other contacts
- **Forwarding automation**: Automatically forward received messages to a specific group or contact
- **Message backup**: Forward important messages to a backup chat
...
|------|--------|----------------|
| 400 | Invalid parameters | Check `phone`, `messageId` and `messagePhone` |
| 401 | Invalid token | Check the `Client-Token` header |
| 404 | Message not found | Check if the `messageId` exists and is valid in the chat specified by `messagePhone` |
...
- You can only forward messages that you have received or sent
- Deleted messages cannot be forwarded
- The original message format is preserved (text, media, etc.)

:::warning Important

Make sure the `messageId` exists and belongs to your instance before trying to forward.

:::

## <Icon name="Info" size="md" /> Important notes {#notas-importantes}

- The forwarded message retains the original format (text, image, video, etc.)
- The recipient will see that the message was forwarded (visual indicator in WhatsApp)
- Use the `messageId` of messages received through webhooks or listed in chats
- Forwarding preserves the original content, including media and formatting

## <Icon name="ArrowRight" size="md" /> Next steps {#proximos-passos}

- [Delete message](/docs/messages/deletar) - Remove sent messages
