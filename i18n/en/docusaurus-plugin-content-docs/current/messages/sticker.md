...
# <Icon name="Smile" size="lg" /> Send Sticker

Send a WhatsApp sticker to a recipient using the Z-API. Stickers are animated or static images used to express emotions and reactions.

![Example message with sticker](/img/send-message-sticker.jpeg)
...
| `sticker` | string | Yes | URL of the sticker or base64 string. Supported formats: WEBP, PNG. |
...
| `delayMessage` | number | No | Controls the wait time (in seconds) before sending the next message. Values between 1 and 15 seconds. If not specified, the system uses an automatic delay of 1 to 3 seconds. Useful when sending multiple stickers in sequence to avoid blocks. |
| `stickerAuthor` | string | No | Author name of the sticker. Appears as metadata when the sticker is viewed. Useful for giving credit to the sticker creator or identifying the origin. |

## <Icon name="FileText" size="md" /> Supported Formats {#formatos-suportados}

You can send stickers using:

- **URL**: `https://exemplo.com/sticker.webp` - Publicly accessible URL that returns the sticker directly
- **Base64**: `data:image/webp;base64,UklGRiQAAABXRUJQVlA4...` - Sticker encoded in base64

**Important:** If using Base64, you need to add the prefix `data:image/png;base64,` or `data:image/webp;base64,` before the base64 code.
...
1. **`phone`**: The contact number that will receive the sticker. Use the full format: DDI + DDD + Number (ex: `5511999999999`). 
2. **`sticker`**: The field where you will paste the **public URL** of your sticker or the base64 string. **Recommendation:** Use URL whenever possible, as it is simpler and more reliable.
...
4. **`delayMessage`**: If you are sending multiple stickers in sequence, use this field to space the send (between 1 and 15 seconds). This helps avoid blocks and makes communication more natural.

5. **`stickerAuthor`**: Author name or origin of the sticker. Appears as metadata when the sticker is viewed. Useful for giving credit to the creator or identifying the source.

**Tip:** In most cases, you only need to fill in `phone` and `sticker`. The other fields are optional and can be left blank.

**Sticker Formats:**

- **WEBP**: Recommended format by WhatsApp (best compression)
- **PNG**: Alternative supported format
- **Maximum size**: 100KB (recommended by WhatsApp)

**Where to find stickers:**

- Use your own catalog or custom packages
- Ensure the sticker URL is public and accessible
...
|--------|-----------------------|----------------------------------------------------|
| 400 | Invalid parameters | Check `phone` and `sticker` |
| 401 | Invalid token | Check the header `Client-Token` |
| 413 | File too large | Reduce the sticker size (max 100KB) |
...
- **Maximum size**: 100KB per sticker
- **Supported formats**: WEBP (recommended), PNG
- **Dimensions**: Recommended 512x512 pixels for static or animated stickers
- **URLs**: Should be publicly accessible and return the sticker directly. Avoid URLs that require authentication or complex redirects

:::tip Tip

Use format WEBP for better compression and quality. Animated stickers should be in the WEBP animated format.

:::

## <Icon name="Info" size="md" /> Important Notes {#notas-importantes}

- Stickers are different from regular images and appear larger in WhatsApp
- For better performance, use URLs instead of base64 when possible
- Large stickers may take longer to process
- Ensure that the sticker URLs are publicly accessible so that WhatsApp can download them
