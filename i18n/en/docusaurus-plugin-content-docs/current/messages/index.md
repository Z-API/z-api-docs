---
title: Messages
sidebar_position: 0
description: Explore all types of messages you can send, from simple text to interactive content.
---

import styles from './index.module.css';

## <Icon name="MessageSquare" size="lg" /> Messages

<div className={styles.heroSection}>

Explore all types of messages you can send through Z-API, from simple text to rich and interactive experiences. Choose the right type for each situation and create more engaging conversations with your customers.

</div>

---

## <Icon name="FileText" size="md" /> Basic Messages

Start with the essentials. These are the most fundamental messages for direct communication.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="FileText" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Overview</h3>
  </div>
  <div className={styles.cardDescription}>
    Welcome to the Z-API messages section! Learn the fundamental concepts that apply to all message types.
  </div>
  <a href="/docs/messages/introducao" className={styles.cardLink}>
    View introduction →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Text" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Text</h3>
  </div>
  <div className={styles.cardDescription}>
    The most fundamental message type. Allows sending any text content to a WhatsApp contact.
  </div>
  <a href="/docs/messages/texto-simples" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Forward" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Forward</h3>
  </div>
  <div className={styles.cardDescription}>
    Forward an existing message to one or more recipients using the Z-API.
  </div>
  <a href="/docs/messages/reencaminhar" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="SmilePlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Add Reaction</h3>
  </div>
  <div className={styles.cardDescription}>
    Add an emoji reaction to an existing message. Reactions allow quick responses with emojis.
  </div>
  <a href="/docs/messages/reacao" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="Image" size="md" /> Media

Make your messages more visual and engaging with images, videos, audio, and documents.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Image" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Image</h3>
  </div>
  <div className={styles.cardDescription}>
    Send images to make your messages more visual and informative. Include an optional caption for context.
  </div>
  <a href="/docs/messages/imagem" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Video" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Video</h3>
  </div>
  <div className={styles.cardDescription}>
    Videos are a powerful way to engage your audience. Demonstrate products, tell stories, or provide visual support.
  </div>
  <a href="/docs/messages/video" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="AudioLines" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Audio</h3>
  </div>
  <div className={styles.cardDescription}>
    Send audio messages for more personal and direct communication. Supports audio files and voice messages.
  </div>
  <a href="/docs/messages/audio" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="File" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Document</h3>
  </div>
  <div className={styles.cardDescription}>
    Send various files such as PDFs, spreadsheets, and documents. Ideal for sharing detailed information and contracts.
  </div>
  <a href="/docs/messages/documentos" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Sticker" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Sticker</h3>
  </div>
  <div className={styles.cardDescription}>
    Send animated or static stickers to express emotions and reactions in a fun way.
  </div>
  <a href="/docs/messages/sticker" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Image" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send GIF</h3>
  </div>
  <div className={styles.cardDescription}>
    Send GIF animations that play automatically on WhatsApp to make your messages more dynamic.
  </div>
  <a href="/docs/messages/gif" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="MousePointerClick" size="md" /> Interactive Messages

Create rich and engaging experiences with buttons, lists, carousels, and more interactive features.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MousePointerClick" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Interactive</h3>
  </div>
  <div className={styles.cardDescription}>
    Explore all types of interactive messages: buttons, option lists, carousels, and more.
  </div>
  <a href="/docs/messages/botoes" className={styles.cardLink}>
    View category →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="List" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Option List</h3>
  </div>
  <div className={styles.cardDescription}>
    Create interactive menus with option lists to guide users through service flows.
  </div>
  <a href="/docs/messages/lista-opcoes" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ImagePlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Carousel</h3>
  </div>
  <div className={styles.cardDescription}>
    Create interactive image carousels to showcase multiple products or options visually.
  </div>
  <a href="/docs/messages/carrossel" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="Link" size="md" /> Other Formats

Share links, locations, contacts, and create polls to gather feedback from your customers.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Link" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Link</h3>
  </div>
  <div className={styles.cardDescription}>
    Send links with automatic preview. WhatsApp automatically generates a preview with title, description, and image.
  </div>
  <a href="/docs/messages/link" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MapPin" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Location</h3>
  </div>
  <div className={styles.cardDescription}>
    Send a geographic location that appears as a point on the WhatsApp map.
  </div>
  <a href="/docs/messages/localizacao" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Contact" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Contact</h3>
  </div>
  <div className={styles.cardDescription}>
    Send contact information (vCard) that can be saved directly in WhatsApp.
  </div>
  <a href="/docs/messages/contato" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Users" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Multiple Contacts</h3>
  </div>
  <div className={styles.cardDescription}>
    Send the same message to multiple recipients at once for efficient mass communication.
  </div>
  <a href="/docs/messages/varios-contatos" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="BarChart3" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Poll</h3>
  </div>
  <div className={styles.cardDescription}>
    Create interactive polls to gather opinions and feedback. Allows voting on one or multiple options.
  </div>
  <a href="/docs/messages/enquete" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="Briefcase" size="md" /> Business Features

WhatsApp Business functionalities for sales, catalogs, and order management.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Package" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Product</h3>
  </div>
  <div className={styles.cardDescription}>
    Send product information from your WhatsApp Business catalog with image, price, and description.
  </div>
  <a href="/docs/messages/produto" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Package" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Catalog</h3>
  </div>
  <div className={styles.cardDescription}>
    Send a link to your complete WhatsApp Business catalog for organized product browsing.
  </div>
  <a href="/docs/messages/catalogo" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShoppingCart" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Order</h3>
  </div>
  <div className={styles.cardDescription}>
    Send complete orders with catalog or customized products. Ideal for e-commerce and billing.
  </div>
  <a href="/docs/messages/aprovacao-pedido" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="Settings" size="md" /> Actions and Management

Manage your messages: delete, mark as read, reply, and pin important messages.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Trash2" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Delete Message</h3>
  </div>
  <div className={styles.cardDescription}>
    Delete a sent message. The message will be removed for all participants in the conversation.
  </div>
  <a href="/docs/messages/deletar" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="CheckCircle" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Mark as Read</h3>
  </div>
  <div className={styles.cardDescription}>
    Mark messages in a chat as read to remove 'unread' indicators and update status.
  </div>
  <a href="/docs/messages/ler" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Reply" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Reply to Message</h3>
  </div>
  <div className={styles.cardDescription}>
    Reply to a specific message. The reply will appear as a threaded response in WhatsApp.
  </div>
  <a href="/docs/messages/responder" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Pin" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Pin/Unpin</h3>
  </div>
  <div className={styles.cardDescription}>
    Pin or unpin messages in a chat. Pinned messages appear at the top and are highlighted.
  </div>
  <a href="/docs/messages/fixar-desafixar" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

## <Icon name="Zap" size="md" /> New Features

Explore the latest features added to the API, including new interactive message types and events.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MousePointerClick" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>OTP Button</h3>
  </div>
  <div className={styles.cardDescription}>
    Send authentication codes (OTP) with an easy copy button. Great for 2FA.
  </div>
  <a href="/docs/messages/botao-otp" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="CreditCard" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>PIX Button</h3>
  </div>
  <div className={styles.cardDescription}>
    Send copy-and-paste PIX codes with a dedicated button to facilitate payments.
  </div>
  <a href="/docs/messages/botao-pix" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShoppingBag" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Order Status</h3>
  </div>
  <div className={styles.cardDescription}>
    Update order status with structured messages for your customers.
  </div>
  <a href="/docs/messages/atualizacao-status-pedido" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="UserPlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Channel Admin Invite</h3>
  </div>
  <div className={styles.cardDescription}>
    Send invitations to add new administrators to your channels (newsletter).
  </div>
  <a href="/docs/messages/convite-admin-canal" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Calendar" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Send Event</h3>
  </div>
  <div className={styles.cardDescription}>
    Send event invitations with date, time, location, and video call option.
  </div>
  <a href="/docs/messages/evento" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Edit" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Edit Event</h3>
  </div>
  <div className={styles.cardDescription}>
    Edit information of already sent events, notifying participants.
  </div>
  <a href="/docs/messages/editar-evento" className={styles.cardLink}>
    View guide →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MessageCircle" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Reply to Event</h3>
  </div>
  <div className={styles.cardDescription}>
    Reply to event invitations via API (Attend, Maybe, Decline).
  </div>
  <a href="/docs/messages/responder-evento" className={styles.cardLink}>
    View guide →
  </a>
</div>

</div>

---

:::tip Next Steps

Start with the [Message Overview](/docs/messages/introducao) to understand the fundamental concepts that apply to all message types.

:::