...
# <Icon name="QrCode" size="lg" /> Get QRCode

Obtain the QR Code to connect your instance to WhatsApp through the Z-API.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituacao}

As in WhatsApp Web, you need to read a QR Code or use a phone number to connect to the Z-API.

There are 2 ways you can use to perform this connection:

- Connect via our admin panel or
- Provide the experience within your own application through the methods described in this section.

You may opt for one of the available methods to read the WhatsApp QR Code.
...
You can enter the generated code through the API directly into WhatsApp, on the same tab where the QR Code is read, clicking on "Connect with phone number".
...
#### For `/phone-code/{phone}`

| Parameter | Type | Required | Description |
|-----------|------|----------|------------|
| `phone` | string | Yes | Phone number in the format DDI + DDD + NUMBER (ex: `5511999999999`) |
...
Returns the bytes of the QR Code image (PNG format).

### 200 OK - QR Code (Image Base64) {#200-ok-image}
...
### 200 OK - Phone Code {#200-ok-phone}
...
| `409` | Already connected | The instance is already connected, no QR Code needed |
...
If you chose to implement QR Code reading in your application, you need to know that WhatsApp invalidates the QR Code every 20 seconds.

If you call the method and are already connected, it will not allow you to connect again.

Once connected, you can start using the Z-API methods to manipulate your WhatsApp.

:::

:::important Recommendations

- **Create a method with intervals between 10 and 20 seconds** to call the API and get the new QR Code.
- **If the user does not read the QR Code after 3 calls**, interrupt the flow and add a button requesting interaction from the same user to avoid unnecessary API calls for WhatsApp.

:::

- Use the endpoint `/qr-code` when you need to render the QR Code in a native component of your programming language
- Use the endpoint `/qr-code/image` when you need to display the QR Code as an HTML image (`<img src="data:image/png;base64,...">`)
- Use the endpoint `/phone-code/{phone}` when you want to allow connection via numeric code instead of QR Code
- The QR Code expires every 20 seconds, so you should update periodically if the user has not read it yet