...
# <Icon name="Activity" size="lg" /> Instance Status

Check if your instance is connected to WhatsApp through the Z-API.

---

## <Icon name="Info" size="md" /> Conceptualization {#conceituacao}

This method allows you to determine if your instance is connected to a WhatsApp account. It returns information about the connection status and possible errors.
...
|-------|------|------------|
| `connected` | boolean | Indicates if your number is connected to the Z-API |
| `error` | string | Provides details in case any of the attributes are not `true`. Possible values: `'You are already connected.'`, `'You need to restore the session.'`, `'You are not connected.'` |
| `smartphoneConnected` | boolean | Indicates if the physical device is connected to the internet |
...
- The field `smartphoneConnected` indicates if the physical device is connected to the internet
- The field `error` contains specific messages about the connection status when there are issues
- Use this endpoint to check the connection status before performing operations that require an active connection
- If `connected` is `false`, you will need to read the QR Code again to reconnect