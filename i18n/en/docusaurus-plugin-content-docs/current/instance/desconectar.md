...
Disconnect your instance from WhatsApp in a controlled manner. Use this endpoint when you need to temporarily disconnect or before deleting an instance.
...
| 409 | Instance already disconnected | The instance is already disconnected |
...
// Desconectar instância com tratamento seguro de erros
async function disconnectInstance() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/disconnect`;
...
async function disconnectInstance(): Promise<DisconnectResponse> {
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/disconnect"
...
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.get('value'):
        print('Instância desconectada com sucesso')
    else:
        print('Erro ao desconectar instância')
...
# ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
# Desconectar instância via cURL
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/disconnect" \
...
function disconnectInstance() {
  return new Promise((resolve, reject) => {
    const path = `/instances/${encodeURIComponent(instanceId)}/disconnect`;
...
// Middleware para desconectar instância
app.use(async (ctx) => {
  if (ctx.path === '/api/disconnect' && ctx.method === 'POST') {
    try {
      // ⚠️ SEGURANÇA: Sempre use HTTPS
      const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/disconnect`;
...
public class DisconnectInstance {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/disconnect",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                System.out.println("Instância desconectada com sucesso");
...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        {
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            var url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/disconnect";
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Instância desconectada com sucesso");
...
func main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/disconnect", instanceId)
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Println("Instância desconectada com sucesso")
...
# ⚠️ SEGURANÇA: Sempre use HTTPS
url = URI("https://api.z-api.io/instances/#{CGI.escape(instance_id)}/disconnect")
...
begin
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['value']
      puts 'Instância desconectada com sucesso'
    end
    puts result.to_json
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? instanceId)/disconnect") else {
    print("URL inválida")
    exit(1)
}
...
# ⚠️ SEGURANÇA: Sempre use HTTPS
$url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/disconnect"
...
try {
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if ($response.value) {
        Write-Host "Instância desconectada com sucesso"
    }
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/disconnect HTTP/1.1
Host: api.z-api.io
...
int main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    std::string url = "https://api.z-api.io/instances/" + instanceId + "/disconnect";
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Instância desconectada com sucesso" << std::endl;
...
int main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    char url[512];
    snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/disconnect", instanceId);
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                printf("Instância desconectada com sucesso\n");
                printf("%s\n", responseData);
            } else {
                printf("Erro HTTP %ld\n", responseCode);
            }
...
- You need to temporarily disconnect the instance
- You are deleting an instance (disconnect before deleting)
- You want to force a new connection (disconnect and then reconnect)
- You need to pause message sending temporarily

## Important {#importante}

:::warning Attention

After disconnecting, the instance will not be able to send or receive messages until it is reconnected. You will need to read the QR Code again to reconnect.

:::

## Notes {#notas}

- The disconnection is immediate and cannot be reversed without reconnecting
- After disconnecting, you will receive a webhook for disconnection
- Pending messages in the queue will not be sent after the disconnection
- To reconnect, use the QR Code endpoint and read the code with WhatsApp
- Disconnecting is useful for maintenance or when you do not need an active instance