...
Restart your WhatsApp instance to resolve connection issues or update the session. Use this endpoint when the instance is experiencing problems or after configuration changes.
...
| 409 | Instance already restarting | Wait for the previous restart to complete |
...
- The instance is experiencing connection issues
- You need to update the WhatsApp connection
- There are configuration changes that require a restart
...
Restarting the instance temporarily disconnects WhatsApp. If the connection is lost during the restart, you will need to scan the QR Code again to reconnect.

:::

## Notes {#notas}

- Restart may take a few seconds to complete
- After the restart, the instance will attempt to reconnect automatically
- If the automatic reconnection fails, you will receive a disconnection webhook
- Monitor the instance status after the restart to confirm the reconnection
- Avoid frequently restarting the instance as it may cause disconnections