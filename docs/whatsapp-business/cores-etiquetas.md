...
# <Icon name="Palette" size="lg" /> Cores de Etiquetas

Liste as cores disponíveis para etiquetas no WhatsApp Business.
...
- **Personalização Visual**: Escolha cores para organizar visualmente suas etiquetas
- **Categorização**: Use cores diferentes para diferentes categorias de contatos
- **Identificação Rápida**: Identifique rapidamente tipos de contatos pelas cores

---

## Para Usuários No-Code

Se você está usando uma plataforma de automação como n8n ou Make, use este endpoint para obter as cores disponíveis antes de criar ou editar etiquetas.
...
Através deste método, é possível listar as cores de etiquetas disponíveis no WhatsApp Business. As cores são retornadas como valores hexadecimais indexados numericamente.

**Importante**: As cores podem variar entre diferentes plataformas (Android / iPhone).
...
// Buscar cores de etiquetas com tratamento seguro de erros
async function getTagColors() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/business/tags/colors`;
    ...
    if (result && typeof result === 'object') {
      console.log(`Total de cores disponíveis: ${Object.keys(result).length}`);
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao buscar cores de etiquetas:', error.message);
    throw error;
  }
}
...
interface TagColorsResponse {
  [key: string]: string; // Índice numérico -> cor hexadecimal
}

// Função para buscar cores de etiquetas
async function getTagColors(): Promise<TagColorsResponse> {
...
getTagColors()
  .then((colors) => console.log(`Total de cores: ${Object.keys(colors).length}`))
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/business/tags/colors"
...
try:
    ...
    if isinstance(result, dict):
        print(f'Total de cores disponíveis: {len(result)}')
...
# Buscar cores de etiquetas via cURL
curl -X GET \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/business/tags/colors" \
...
// Buscar cores de etiquetas
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/business/tags/colors`);
...
// Rota para buscar cores de etiquetas
app.get('/business/tags/colors', async (req, res) => {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/business/tags/colors`);
...
// Rota para buscar cores de etiquetas
router.get('/business/tags/colors', async (ctx) => {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/business/tags/colors`);
...
app.on('error', (err, ctx) => {
  console.error('Erro ao buscar cores de etiquetas:', err.message);
});
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/business/tags/colors",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Cores de etiquetas obtidas com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class GetTagColors
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        {
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/business/tags/colors";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Cores de etiquetas obtidas com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/business/tags/colors",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Total de cores disponíveis: %d\n", len(result))
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        if (is_array($result)) {
            echo "Total de cores disponíveis: " . count($result) . "\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/business/tags/colors")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.is_a?(Hash)
      puts "Total de cores disponíveis: #{result.length}"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/business/tags/colors") else {
    ...
    if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
        if let data = data {
            do {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                if let colors = result {
                    print("Total de cores disponíveis: \(colors.count)")
                }
            } catch {
...
try {
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/business/tags/colors"
    ...
    if ($response -is [Hashtable]) {
        Write-Host "Total de cores disponíveis: $($response.Count)"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
GET https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/business/tags/colors HTTP/1.1
Host: api.z-api.io
...
int main() {
    ...
    if (curl) {
        std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/business/tags/colors";
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            std::cout << "Cores de etiquetas obtidas com sucesso" << std::endl;
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/business/tags/colors",
                 instanceId, instanceToken);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Cores de etiquetas obtidas com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
O retorno é um objeto onde cada chave é um índice numérico e o valor é a cor hexadecimal correspondente.

| Atributo | Tipo | Descrição |
|----------|------|-----------|
| `{INDICE_DA_COR}` | string | Hexadecimal da cor (ex: `"#FF9485"`) |
...
| Código | Descrição |
|--------|-----------|
| `405` | Método HTTP incorreto. Verifique se está usando `GET` |
...
:::tip Variação de Cores

As cores podem variar entre diferentes plataformas (Android / iPhone). Sempre teste visualmente antes de usar em produção.

:::

- **Uso em Etiquetas**: Use os índices retornados ao criar ou editar etiquetas
- **Compatibilidade**: Verifique se as cores funcionam bem em diferentes dispositivos
- **Acessibilidade**: Considere contraste e legibilidade ao escolher cores

---

## Próximos Passos

- [Criar Etiqueta](/docs/whatsapp-business/criar-etiqueta) - Crie etiquetas com cores
- [Editar Etiqueta](/docs/whatsapp-business/editar-etiqueta) - Edite cores de etiquetas existentes
- [Etiquetas](/docs/whatsapp-business/buscar-etiquetas) - Gerencie suas etiquetas
