...
# <Icon name="PackageMinus" size="lg" /> Remover Produtos da Coleção

Remova produtos de uma coleção existente do seu catálogo do WhatsApp Business.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite que você remova produtos de uma coleção existente do seu catálogo. Você pode remover múltiplos produtos de uma vez, especificando seus IDs.

:::warning Atenção
Ao adicionar ou remover produtos de uma **coleção**, o ID da mesma é alterado pelo WhatsApp. Isso significa que, ao remover um produto da coleção e tentar fazer qualquer outra operação utilizando o **ID** "antigo", resultará em um não funcionamento da rota. Lembre-se então de utilizar o ID retornado por essa mesma rota, o qual já é o ID atualizado para as operações seguintes.
:::
...
|----------|------|-----------|
| `collectionId` | string | ID da coleção da qual os produtos serão removidos |
| `productIds` | array[string] | Array com os IDs dos produtos que serão removidos da coleção. Ex: `["6643149779134830", "6988917394481455"]` |
...
O `collectionId` retornado é o **novo ID** da coleção após a remoção dos produtos. Sempre use este ID atualizado para operações subsequentes, pois o ID anterior não funcionará mais.
:::
...
- **ID da coleção muda**: O ID da coleção é alterado após remover produtos. Sempre use o ID retornado na resposta
- **Múltiplos produtos**: Você pode remover vários produtos de uma vez, passando um array com os IDs
- **Produtos inexistentes**: Se um produto não estiver na coleção, a operação ainda será bem-sucedida
