...
# <Icon name="Mail" size="lg" /> Alterar Email da Empresa

Atualize o email de contato da sua empresa no WhatsApp Business.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite alterar o email de contato da empresa/companhia no seu perfil do WhatsApp Business. O email é exibido publicamente no perfil da sua conta Business e permite que clientes entrem em contato por email.
...
|----------|------|-----------|
| `value` | string | Email da empresa (deve estar em formato válido de email) |

:::important Formato do Email
O campo enviado deve estar em um formato válido de email. Preencher esse valor com um texto que não esteja no formato de email resultará em um erro.
:::
...
| `400` | Email inválido | Verifique se o email está em formato válido (ex: `usuario@dominio.com`) |
...
- **Email público**: O email é exibido publicamente no perfil da sua conta Business
- **Formato válido**: O email deve estar em formato válido (ex: `usuario@dominio.com`)
- **Remover email**: Para remover o email, envie `value` como string vazia (`""`)
