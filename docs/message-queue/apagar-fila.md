...
Remova todas as mensagens da fila de uma instância através da API do Z-API. Use este endpoint para esvaziar completamente a fila de mensagens.
...
|--------|-----------------------|----------------------------------------------------|
| 400 | Parâmetros inválidos | Verifique o `instanceId` |
...
// Função para apagar fila com tratamento seguro de erros
async function clearQueue() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/queue`;
    ...
    if (result.value) {
      ...
      console.error('Erro ao esvaziar fila');
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao esvaziar fila:', error.message);
    throw error;
  }
}
...
// Função para apagar fila
async function clearQueue(): Promise<ClearQueueResponse> {
...
# Apagar fila com tratamento seguro de erros
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.get('value'):
        print('Fila esvaziada com sucesso')
    else:
        print('Erro ao esvaziar fila')
...
# ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
# Apagar fila via cURL
curl -X DELETE \
  "https://api.z-api.io/instances/${INSTANCE_ID}/queue" \
...
// Apagar fila
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/queue`);
...
app.delete('/queue', async (req, res) => {
  try {
    ...
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao apagar fila:', error.message);
...
app.on('error', (err, ctx) => {
  console.error('Erro ao apagar fila:', err.message);
});
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/queue",
...
public class ClearQueue
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        {
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/queue";
...
func main() {
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/queue", instanceId)
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Fila esvaziada com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        if ($result['value'] ?? false) {
            echo "Fila esvaziada com sucesso\n";
        } else {
            echo "Erro ao esvaziar fila\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/queue")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['value']
      puts "Fila esvaziada com sucesso"
    else
      puts "Erro ao esvaziar fila"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/queue") else {
...
try {
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/queue"
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if ($response.value) {
        Write-Host "Fila esvaziada com sucesso"
    } else {
        Write-Host "Erro ao esvaziar fila"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
DELETE https://api.z-api.io/instances/SUA_INSTANCIA/queue HTTP/1.1
Host: api.z-api.io
Client-Token: seu-token-de-seguranca
Content-Type: application/json

Nota: Este endpoint não requer instanceToken na URL, mas todas as chamadas Z-API devem ter instanceId, instanceToken e clientToken configurados.
```
...
int main() {
    try {
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/queue";
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Fila esvaziada com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/queue",
                 instanceId);
...
- ⚠️ **ATENÇÃO**: Esta ação é permanente e remove TODAS as mensagens da fila
