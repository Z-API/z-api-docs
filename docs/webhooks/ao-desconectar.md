...
# Webhook: Ao desconectar

Receba notificações quando sua instância do Z-API se desconecta do WhatsApp. Use este webhook para detectar desconexões e implementar lógica de reconexão.

## Evento {#evento}

O webhook envia um payload JSON com o tipo `DisconnectedCallback` quando a instância se desconecta do WhatsApp.
...
## Razões de desconexão {#razoes-de-desconexao}

O campo `error` contém a descrição do erro que causou a desconexão. Exemplos comuns incluem desconexão do dispositivo, conflito de sessões, ou banimento.
...
Este webhook é chamado quando:

- A instância perde conexão com o WhatsApp
- A instância é desconectada manualmente através da API
- A sessão do WhatsApp expira
- O usuário faz logout no WhatsApp
- Ocorre um timeout na conexão
...
- **Estado da instância**: Atualize o estado da instância no seu sistema quando receber este webhook
- **Mensagens pendentes**: Pause o envio de mensagens quando a instância estiver desconectada
- **Reconexão**: Implemente lógica para tentar reconectar automaticamente quando apropriado

## Notas {#notas}

- Após receber este webhook, a instância não poderá enviar ou receber mensagens
- Para reconectar, você precisará gerar um novo QR Code e ler com o WhatsApp
- Algumas desconexões são temporárias e podem ser resolvidas com reconexão
- Monitore este webhook para detectar problemas de conexão recorrentes
- Implemente alertas para desconexões frequentes que podem indicar problemas
