...
# <Icon name="AlertTriangle" size="lg" /> Denunciar Contato

Denuncie um contato no WhatsApp através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite denunciar um contato no WhatsApp. Use esta funcionalidade para reportar contatos que estão enviando spam, conteúdo inadequado ou violando as políticas do WhatsApp.
...
|-----------|------|-----------|
| `phone` | string | Número de telefone que você deseja denunciar no formato DDI DDD NÚMERO. Ex: `551199999999`. **IMPORTANTE**: Envie somente números, sem formatação ou máscara |
...
|-------|------|-----------|
| `success` | boolean | `true` se a denúncia foi bem-sucedida, `false` em caso de falha |
| `error` | string | Mensagem de erro, caso ocorra (apenas quando `success` é `false`) |
...
- **Uso responsável**: Use esta funcionalidade apenas para denunciar contatos que realmente violam as políticas do WhatsApp
- **Efeito**: A denúncia é enviada ao WhatsApp para análise
- **Formato**: O número deve estar no formato DDI DDD NÚMERO, sem formatação ou máscara
- **Sem body**: Esta requisição não requer body, apenas o número na URL
