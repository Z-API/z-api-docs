...
# <Icon name="Ban" size="lg" /> Bloquear ou Desbloquear Contato

Bloqueie ou desbloqueie um contato no WhatsApp através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite bloquear ou desbloquear um contato no WhatsApp. Quando um contato está bloqueado, ele não consegue enviar mensagens para você e você não recebe notificações dele.
...
|----------|------|-----------|
| `phone` | string | Número de telefone que você deseja bloquear ou desbloquear no formato DDI DDD NÚMERO. Ex: `551199999999`. **IMPORTANTE**: Envie somente números, sem formatação ou máscara |
| `action` | string | Ação a ser executada: `"block"` para bloquear ou `"unblock"` para desbloquear |
...
function validateAction(action) {
  if (action !== 'block' && action !== 'unblock') {
    throw new Error('Ação inválida. Use "block" ou "unblock"');
  }
  return action;
}
...
const rawAction = 'block'; // Use 'block' para bloquear ou 'unblock' para desbloquear
...
// Bloquear/desbloquear contato com tratamento seguro de erros
async function modifyBlockedContact() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/contacts/modify-blocked`;
 ...
 'Content-Type': 'application/json',
    ...
    console.error('Erro ao bloquear/desbloquear contato:', error.message);
    throw error;
  }
}

// Executar função
modifyBlockedContact();
```
...
def validate_action(action: str) -> str:
    """Valida ação (block ou unblock)."""
    if action not in ['block', 'unblock']:
        raise ValueError('Ação inválida. Use "block" ou "unblock"')
    return action

# Dados da requisição com validação
raw_phone = '5544999999999'
raw_action = 'block'  # Use 'block' para bloquear ou 'unblock' para desbloquear
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/contacts/modify-blocked"
...
# ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
PHONE="5544999999999"
ACTION="block"  # Use "block" para bloquear ou "unblock" para desbloquear

# Bloquear/desbloquear contato via cURL
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/contacts/modify-blocked" \
...
function validateAction(action: string): 'block' | 'unblock' {
  if (action !== 'block' && action !== 'unblock') {
    throw new Error('Ação inválida. Use "block" ou "unblock"');
  }
  return action as 'block' | 'unblock';
}

// Dados da requisição com validação
const requestData: BlockContactRequest = {
  phone: validatePhoneNumber('5544999999999'),
  action: validateAction('block'),
};

// Função para bloquear/desbloquear contato
async function modifyBlockedContact(): Promise<BlockContactResponse> {
...
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/contacts/modify-blocked`);
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      ...
      // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
      console.log('Ação executada com sucesso:', result.value);
    } else {
      // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// Rota para bloquear/desbloquear contato
app.post('/contacts/modify-blocked', async (req, res) => {
  try {
    // Dados da requisição com validação
    const rawPhone = req.body.phone || '5544999999999';
    const rawAction = req.body.action || 'block';
...
// Rota para bloquear/desbloquear contato
router.post('/contacts/modify-blocked', async (ctx) => {
  try {
    // Dados da requisição com validação
    const rawPhone = ctx.request.body.phone || '5544999999999';
    const rawAction = ctx.request.body.action || 'block';
...
app.on('error', (err, ctx) => {
  console.error('Erro ao bloquear/desbloquear contato:', err.message);
});
...
public class ModifyBlockedContact {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    private static String validateAction(String action) {
        if (!action.equals("block") && !action.equals("unblock")) {
            throw new IllegalArgumentException("Ação inválida. Use \"block\" ou \"unblock\"");
        }
        return action;
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            String action = validateAction("block");

            // ⚠️ SEGURANÇA: Sempre use HTTPS
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/contacts/modify-blocked",
                java.net.URLEncoder.encode(INSTANCE_ID, StandardCharsets.UTF_8),
                java.net.URLEncoder.encode(INSTANCE_TOKEN, StandardCharsets.UTF_8)
            );

            String jsonInputString = String.format("{\"phone\":\"%s\",\"action\":\"%s\"}", phone, action);
...
public class ModifyBlockedContact
{
    ...
    private static string ValidateAction(string action)
    {
        if (action != "block" && action != "unblock")
        {
            throw new ArgumentException("Ação inválida. Use \"block\" ou \"unblock\"");
        }
        return action;
    }
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/contacts/modify-blocked";
...
func validateAction(action string) (string, error) {
    if action != "block" && action != "unblock" {
        return "", fmt.Errorf("ação inválida. Use \"block\" ou \"unblock\"")
    }
    return action, nil
}
...
func main() {
    ...
    action, err := validateAction("block")
    if err != nil {
        fmt.Printf("Erro de validação (ação): %v\n", err)
        return
    }

    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/contacts/modify-blocked",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Ação executada com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
function validateAction($action) {
    if ($action !== 'block' && $action !== 'unblock') {
        throw new Exception('Ação inválida. Use "block" ou "unblock"');
    }
    return $action;
}
...
def validate_action(action)
  unless ['block', 'unblock'].include?(action)
    raise ArgumentError, 'Ação inválida. Use "block" ou "unblock"'
  end
  action
end
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/contacts/modify-blocked")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['value']
      puts "Ação executada com sucesso"
    else
      puts "Erro ao executar ação"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func validateAction(_ action: String) throws -> String {
    guard action == "block" || action == "unblock" else {
        throw NSError(domain: "ValidationError", code: 400, userInfo: [NSLocalizedDescriptionKey: "Ação inválida. Use \"block\" ou \"unblock\""])
    }
    return action
}
...
let action = try validateAction("block")

// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/contacts/modify-blocked") else {
...
function Validate-Action {
    param([string]$Action)
    if ($Action -ne "block" -and $Action -ne "unblock") {
        throw "Ação inválida. Use ""block"" ou ""unblock"""
    }
    return $Action
}
...
try {
    ...
    $action = Validate-Action -Action "block"
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/contacts/modify-blocked"
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if ($response.value) {
        Write-Host "Ação executada com sucesso"
    } else {
        Write-Host "Erro ao executar ação"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/contacts/modify-blocked HTTP/1.1
Host: api.z-api.io
...
std::string validateAction(const std::string& action) {
    if (action != "block" && action != "unblock") {
        throw std::runtime_error("Ação inválida. Use \"block\" ou \"unblock\"");
    }
    return action;
}
...
int main() {
    try {
        ...
        std::string action = validateAction("block");

        std::string jsonData = "{\"phone\":\"" + phone + "\",\"action\":\"" + action + "\"}";

        CURL* curl = curl_easy_init();
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/contacts/modify-blocked";
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Ação executada com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int validateAction(const char* action) {
    return (strcmp(action, "block") == 0 || strcmp(action, "unblock") == 0) ? 1 : 0;
}
...
int main() {
    ...
    if (!validateAction(action)) {
        fprintf(stderr, "Ação inválida. Use \"block\" ou \"unblock\"\n");
        return 1;
    }
    ...
    if (curl) {
        char url[512];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/contacts/modify-blocked",
                 instanceId, instanceToken);

        char jsonData[256];
        snprintf(jsonData, sizeof(jsonData), "{\"phone\":\"%s\",\"action\":\"%s\"}", cleanedPhone, action);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Ação executada com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
|-------|------|-----------|
| `value` | boolean | `true` se a ação foi bem-sucedida, `false` em caso de falha |
...
| `400` | Parâmetros inválidos | Verifique se `action` é `"block"` ou `"unblock"` e se o número está no formato correto |

---

## <Icon name="Info" size="md" /> Observações {#observacoes}

- **Ação**: Use `"block"` para bloquear ou `"unblock"` para desbloquear
- **Efeito**: Quando bloqueado, o contato não consegue enviar mensagens e você não recebe notificações
- **Reversível**: Você pode desbloquear a qualquer momento usando `"unblock"`
- **Formato**: O número deve estar no formato DDI DDD NÚMERO, sem formatação
