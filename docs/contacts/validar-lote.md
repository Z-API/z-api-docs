...
# <Icon name="ListChecks" size="lg" /> Validar Números em Lote

Valide múltiplos números de telefone de uma vez para verificar se possuem WhatsApp ativo.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Diferente do método [Verificar Número com WhatsApp](/docs/contacts/numero-whatsapp) que valida individualmente através de uma requisição GET, esta API oferece uma verificação em lote através de uma requisição POST, permitindo validar até 50 mil números por requisição.
...
Este método continua a ser a escolha ideal quando você precisa verificar se um número possui WhatsApp, especialmente útil para a **validação de formulários**. 

No entanto, é crucial observar que **não é recomendado** utilizar esta API para verificar a existência do número antes de enviar uma mensagem. O Z-API já realiza essa validação automaticamente a cada mensagem enviada, e o uso duplicado deste método pode resultar em problemas.
:::
...
|----------|------|-----------|
| `phones` | array | Array de números de telefone a serem validados no formato DDI DDD NÚMERO. Ex: `["551199999999", "551188888888"]`. **IMPORTANTE**: Envie somente números, sem formatação ou máscara |
...
|-------|------|-----------|
| `exists` | boolean | `true` se o número possui WhatsApp, `false` caso contrário |
...
| `400` | Array vazio ou formato inválido | Verifique se o array `phones` contém números válidos no formato correto |

---

## <Icon name="Info" size="md" /> Observações {#observacoes}

- **Limite**: Máximo de 50 mil números por requisição
- **Formato**: Os números devem estar no formato DDI DDD NÚMERO, sem formatação ou máscara
- **outputPhone**: O número retornado pode incluir o nono dígito se o WhatsApp tiver essa informação
- **Performance**: Mais eficiente que fazer múltiplas requisições individuais
- **Uso recomendado**: Validação de formulários e limpeza de bases de dados
