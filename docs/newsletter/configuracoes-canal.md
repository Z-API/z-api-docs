...
Este método permite alterar as configurações de interação em um canal, como as permissões de reação.
...
|-----------|------|-----------|
| id | string | ID do canal (path `path parameter`). Ex: `123...@newsletter` |
| reactionCodes | string | Opções: `basic` (apenas reações básicas) ou `all` (todas as reações) |
