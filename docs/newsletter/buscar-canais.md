...
Este método retorna uma lista de dados de newsletter baseado na busca realizada usando filtros fornecidos no corpo da requisição.
...
| filters.countryCodes | array string | Array com códigos de países (https://www.iban.com/country-codes) |
...
| name | string | Nome da newsletter |
| description | string | Descrição da newsletter |
| subscribersCount | string | Número de seguidores |
...
:::warning Atributo "cursor" no objeto de resposta
A API do WhatsApp fornece o atributo "limit" para realizar buscas de newsletter, o que significa que existe paginação de resultados. No entanto, a resposta não indica o "cursor" dos registros. Portanto, por enquanto, o atributo "cursor" sempre será "null" até que o WhatsApp implemente essa funcionalidade.
:::
