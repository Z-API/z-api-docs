...
# <Icon name="Image" size="lg" /> Botões com Imagem

Envie imagens acompanhadas de botões de resposta rápida. O usuário pode escolher uma opção clicando em um dos botões, e a resposta será capturada via webhook.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite enviar uma imagem com botões de resposta rápida. Quando o usuário clicar em um botão, a resposta será enviada como uma mensagem que pode ser capturada via webhook, permitindo criar fluxos interativos baseados na escolha do usuário.
...
O atributo `message` não pode ser enviado vazio! Sempre inclua um texto junto com a imagem.
:::

![Exemplo de botões com imagem](/img/send-button-list-image.jpeg)
...
- **`phone`**: O número do destinatário para onde você deseja enviar a imagem com botões. Use o formato completo: DDI + DDD + Número (ex: `5511999999999`). **Importante:** Use apenas números, sem formatação ou máscara. Para grupos, use o ID do grupo.

- **`message`**: O texto que será exibido junto com a imagem. **Este campo é obrigatório e não pode estar vazio!** Use este campo para explicar o que o usuário deve fazer ou fornecer contexto sobre a imagem.

- **`buttonList`**: Um objeto contendo a imagem e os botões. Dentro deste objeto, você precisa configurar:

  - **`image`**: URL pública ou Base64 da imagem que será enviada. A imagem deve ser acessível publicamente ou estar codificada em Base64.
...
- **Mensagem obrigatória**: O campo `message` não pode estar vazio. Sempre inclua um texto explicativo junto com a imagem.
- **URL da imagem**: Certifique-se de que a URL da imagem seja pública e acessível. Teste a URL no navegador antes de usar na automação.
- **Base64**: Se preferir usar Base64, o formato deve ser: `data:image/jpeg;base64,SEU_CODIGO_BASE64` (ou `data:image/png;base64,` para PNG).
...
- **Pesquisas de satisfação**: Enviar imagem de produto/serviço com botões de avaliação
- **Confirmações visuais**: Enviar imagem de confirmação com botões "Confirmar" e "Cancelar"
- **Catálogos interativos**: Enviar imagem de produto com botões "Ver mais", "Comprar", "Favoritar"
- **Suporte visual**: Enviar imagem de instrução com botões de resposta rápida
- **Promoções**: Enviar imagem promocional com botões "Quero", "Ver detalhes", "Compartilhar"
...
Quando o usuário clicar em um botão, você receberá um webhook com a resposta. O webhook incluirá o `id` do botão clicado, permitindo que você identifique qual opção o usuário escolheu. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem).
...
| `message` | string | Texto a ser enviado junto com a imagem. **Não pode ser vazio** |
| `buttonList` | object | Objeto contendo a imagem e os botões |
...
Para saber quando a mensagem foi entregue, lida, ou quando um botão foi clicado, configure um webhook e monitore os eventos. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem).

### Erros comuns {#erros-comuns}

| Código | Motivo | Como resolver |
|--------|-----------------------|----------------------------------------------------|
| 400 | Parâmetros inválidos | Verifique se todos os atributos obrigatórios foram enviados, especialmente se `message` não está vazio, se `buttonList` contém `image` e `buttons`, e se há pelo menos 1 botão |
...
[Webhook ao receber mensagem - Botões com imagem](/docs/webhooks/ao-receber#exemplo-de-retorno-de-botão-com-imagem)

---

## <Icon name="Info" size="md" /> Observações {#observacoes}

- **Imagem**: Pode ser fornecida via URL pública ou Base64
