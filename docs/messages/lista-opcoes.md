...
# <Icon name="List" size="lg" /> Enviar Mensagem com Lista de Opções

Ela permite apresentar um menu rico e estruturado, onde o usuário toca em um botão para abrir uma lista de itens selecionáveis, que podem ser organizados em seções.
...
![Exemplo de lista de opções](/img/send-option-list.jpeg)

---

## <Icon name="Lightbulb" size="md" /> Casos de Uso Comuns

- **Menus de Suporte:** "Selecione o departamento com o qual deseja falar", com seções para "Vendas", "Financeiro" e "Suporte Técnico".
- **Seleção de Produtos:** "Qual categoria de produto você procura?", com uma lista de todas as categorias disponíveis.
- **Agendamento:** "Escolha o melhor horário para sua consulta", listando todos os horários disponíveis.
- **Perguntas Frequentes (FAQ):** Apresentar uma lista das perguntas mais comuns para o usuário selecionar.

---

## <Icon name="FileText" size="md" /> A Estrutura de uma Lista de Opções

Uma lista é composta por algumas partes principais:

1. **Botão de Abertura:** O texto do botão que o usuário clica para ver a lista (ex: "Ver Opções").
2. **Título da Lista:** O título que aparece no topo da tela da lista (ex: "Escolha uma Categoria").
3. **Seções (Opcional):** Você pode agrupar os itens da lista em seções, cada uma com seu próprio título (ex: "Calçados", "Roupas").
4. **Linhas (Rows):** Cada item clicável na lista é uma "linha", contendo um `id` (para sua automação), um `title` (o que o usuário vê) e uma `description` (opcional).

---

## <Icon name="Wand2" size="md" /> Para Usuários No-Code

Em sua ferramenta de automação, você precisará montar uma estrutura aninhada:
...
2. **`message`**: O texto principal que aparecerá acima do botão da lista (ex: "Selecione a melhor opção:").

3. **`optionList`**: Um objeto que contém a configuração da lista. Dentro dele, você configurará:
   - **`title`** (obrigatório): O título da listagem que aparece no topo da lista quando o usuário abre (ex: "Opções disponíveis").
   - **`buttonLabel`** (obrigatório): O texto do botão que o usuário clica para abrir a lista (ex: "Abrir lista de opções").
   - **`options`** (obrigatório): Uma lista (array) de opções. Cada opção terá:
     - **`title`** (obrigatório): O título da opção (o que o usuário vê, ex: "Z-API").
     - **`description`** (obrigatório): A descrição da opção (texto de apoio, ex: "Z-API Asas para sua imaginação").
     - **`id`** (opcional, mas recomendado): Um identificador único (ex: `"1"`, `"opcao_zapi"`). **Este é o valor que sua automação usará para saber qual opção foi selecionada.**
...
### Criar o Gatilho (Trigger) para Receber Seleções

Após o nó de envio, você criará um novo gatilho que "escuta" as seleções:

1. **Configure um webhook** que recebe eventos do Z-API
2. **Crie uma lógica condicional** (como um "IF" ou "Switch") baseada no valor do `selectedRowId` que chega no webhook
3. **Ações baseadas na opção selecionada:**
   - **Se** `selectedRowId` for `opcao_1`, **então** execute a ação correspondente
   - **Se** `selectedRowId` for `opcao_2`, **então** execute outra ação

**Dicas importantes:**

- **Máximo de opções**: O número máximo de opções varia conforme a plataforma, mas geralmente suporta várias opções
- **Título e descrição**: Ambos são obrigatórios para cada opção
- **Identificadores únicos**: Use `id` descritivos e únicos para facilitar a identificação no webhook
- **Botão de abertura**: O `buttonLabel` é o texto que aparece no botão que abre a lista

**Casos de uso comuns:**

- **Menus de Suporte**: "Selecione o departamento com o qual deseja falar", com opções para "Vendas", "Financeiro" e "Suporte Técnico"
- **Seleção de Produtos**: "Qual categoria de produto você procura?", com uma lista de todas as categorias disponíveis
- **Agendamento**: "Escolha o melhor horário para sua consulta", listando todos os horários disponíveis
- **Perguntas Frequentes (FAQ)**: Apresentar uma lista das perguntas mais comuns para o usuário selecionar
...
Para enviar uma mensagem com uma lista de opções, faça uma requisição `POST` para o endpoint abaixo.
...
O corpo da requisição contém o telefone, a mensagem e um objeto `optionList` com o título da lista, o texto do botão e as opções.
...
#### Estrutura do `optionList`

| Campo | Tipo | Obrigatório | Descrição |
|:------ |:----- |:---------- |:----------------------------------------------------------------------- |
| `title` | string | Sim | Título da listagem que aparece no topo da lista |
| `buttonLabel` | string | Sim | Texto do botão que abre a lista |
| `options` | array | Sim | Lista de opções (máximo de opções varia conforme a plataforma) |

#### Estrutura do `option`

| Campo | Tipo | Obrigatório | Descrição |
|:------ |:----- |:---------- |:----------------------------------------------------------------------- |
| `title` | string | Sim | Título da opção (o que o usuário vê) |
| `description` | string | Sim | Descrição da opção (texto de apoio) |
| `id` | string | Não | Identificador único da opção (usado no webhook para identificar qual opção foi selecionada) |

### Recebendo a Resposta do Usuário

Quando o usuário seleciona um item da lista, você recebe uma notificação via webhook com o `selectedRowId`.
...
Use o `selectedRowId` no seu backend para identificar a escolha do usuário e continuar o fluxo.
...
- ⚠️ **Validação:** Valide `phone` (apenas números, 10-15 dígitos), `message` (não vazio, máximo 4096 caracteres), `buttonText` (não vazio, máximo 20 caracteres) e `sections` (estrutura válida) antes de enviar
...
**Rastreamento de Entrega e Seleções:**

Para saber quando a mensagem foi entregue, lida, ou quando uma opção foi selecionada, configure um webhook e monitore os eventos. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber#exemplo-de-retorno-de-texto-lista-de-opcão).

## <Icon name="Link" size="md" /> Veja Também

### Documentação Relacionada

- <Icon name="List" size="sm" /> [Lista de Opções](/docs/messages/lista-opcoes) - Para mais de 3 opções ou categorias
- <Icon name="Send" size="sm" /> [Enviar Mensagens](/docs/messages/introducao) - Visão geral de tipos de mensagens
- <Icon name="Webhook" size="sm" /> [Webhooks](/docs/webhooks/introducao) - Receber respostas do usuário
