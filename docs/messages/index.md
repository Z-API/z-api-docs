---
title: Mensagens
sidebar_position: 0
description: Explore todos os tipos de mensagens que você pode enviar, de textos simples a conteúdos interativos.
---

import styles from './index.module.css';

## <Icon name="MessageSquare" size="lg" /> Mensagens

<div className={styles.heroSection}>

Explore todos os tipos de mensagens que você pode enviar através do Z-API, de textos simples a experiências ricas e interativas. Escolha o tipo certo para cada situação e crie conversas mais engajadoras com seus clientes.

</div>

---

## <Icon name="FileText" size="md" /> Mensagens Básicas

Comece com o essencial. Estas são as mensagens mais fundamentais para comunicação direta.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="FileText" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Visão Geral</h3>
  </div>
  <div className={styles.cardDescription}>
    Bem-vindo à seção de mensagens do Z-API! Aprenda os conceitos fundamentais que se aplicam a todos os tipos de mensagem.
  </div>
  <a href="/docs/messages/introducao" className={styles.cardLink}>
    Ver introdução →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Text" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Texto</h3>
  </div>
  <div className={styles.cardDescription}>
    O tipo de mensagem mais fundamental. Permite enviar qualquer conteúdo em formato de texto para um contato no WhatsApp.
  </div>
  <a href="/docs/messages/texto-simples" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Forward" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Reencaminhar</h3>
  </div>
  <div className={styles.cardDescription}>
    Reencaminhe uma mensagem existente para um ou mais destinatários usando a API do Z-API.
  </div>
  <a href="/docs/messages/reencaminhar" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="SmilePlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Adicionar Reação</h3>
  </div>
  <div className={styles.cardDescription}>
    Adicione uma reação emoji a uma mensagem existente. Reações permitem responder rapidamente com emojis.
  </div>
  <a href="/docs/messages/reacao" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Image" size="md" /> Mídia

Torne suas mensagens mais visuais e engajadoras com imagens, vídeos, áudios e documentos.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Image" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Imagem</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie imagens para tornar suas mensagens mais visuais e informativas. Inclua uma legenda opcional para dar contexto.
  </div>
  <a href="/docs/messages/imagem" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Video" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Vídeo</h3>
  </div>
  <div className={styles.cardDescription}>
    Vídeos são uma forma poderosa de engajar seu público. Demonstre produtos, conte histórias ou ofereça suporte visual.
  </div>
  <a href="/docs/messages/video" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="AudioLines" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Áudio</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie mensagens de áudio para comunicação mais pessoal e direta. Suporta arquivos de áudio e mensagens de voz.
  </div>
  <a href="/docs/messages/audio" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="File" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Documento</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie arquivos diversos como PDFs, planilhas e documentos. Ideal para compartilhar informações detalhadas e contratos.
  </div>
  <a href="/docs/messages/documentos" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Sticker" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Sticker</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie stickers animados ou estáticos para expressar emoções e reações de forma divertida.
  </div>
  <a href="/docs/messages/sticker" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Image" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar GIF</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie animações GIF que se reproduzem automaticamente no WhatsApp para tornar suas mensagens mais dinâmicas.
  </div>
  <a href="/docs/messages/gif" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="MousePointerClick" size="md" /> Mensagens Interativas

Crie experiências ricas e engajadoras com botões, listas, carrosséis e mais recursos interativos.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MousePointerClick" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Interativas</h3>
  </div>
  <div className={styles.cardDescription}>
    Explore todos os tipos de mensagens interativas: botões, listas de opções, carrosséis e muito mais.
  </div>
  <a href="/docs/messages/botoes" className={styles.cardLink}>
    Ver categoria →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="List" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Lista de Opções</h3>
  </div>
  <div className={styles.cardDescription}>
    Crie menus interativos com listas de opções para guiar usuários através de fluxos de atendimento.
  </div>
  <a href="/docs/messages/lista-opcoes" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ImagePlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Carrossel</h3>
  </div>
  <div className={styles.cardDescription}>
    Crie carrosséis de imagens interativos para mostrar múltiplos produtos ou opções de forma visual.
  </div>
  <a href="/docs/messages/carrossel" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Link" size="md" /> Outros Formatos

Compartilhe links, localizações, contatos e crie enquetes para coletar feedback dos seus clientes.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Link" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Link</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie links com preview automático. O WhatsApp gera automaticamente um preview com título, descrição e imagem.
  </div>
  <a href="/docs/messages/link" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MapPin" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Localização</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie uma localização geográfica que aparece como um ponto no mapa do WhatsApp.
  </div>
  <a href="/docs/messages/localizacao" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Contact" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Contato</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie informações de contato (vCard) que podem ser salvas diretamente no WhatsApp.
  </div>
  <a href="/docs/messages/contato" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Users" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Vários Contatos</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie a mesma mensagem para múltiplos destinatários de uma vez para comunicação em massa eficiente.
  </div>
  <a href="/docs/messages/varios-contatos" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="BarChart3" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Enquete</h3>
  </div>
  <div className={styles.cardDescription}>
    Crie enquetes interativas para coletar opiniões e feedback. Permite votação em uma ou múltiplas opções.
  </div>
  <a href="/docs/messages/enquete" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Briefcase" size="md" /> Recursos Comerciais

Funcionalidades do WhatsApp Business para vendas, catálogos e gerenciamento de pedidos.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Package" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Produto</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie informações de produtos do seu catálogo WhatsApp Business com imagem, preço e descrição.
  </div>
  <a href="/docs/messages/produto" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Package" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Catálogo</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie um link para seu catálogo WhatsApp Business completo para navegação organizada de produtos.
  </div>
  <a href="/docs/messages/catalogo" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShoppingCart" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Pedido</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie pedidos completos com produtos do catálogo ou customizados. Ideal para e-commerce e cobranças.
  </div>
  <a href="/docs/messages/aprovacao-pedido" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Settings" size="md" /> Ações e Gerenciamento

Gerencie suas mensagens: delete, marque como lida, responda e fixe mensagens importantes.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Trash2" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Deletar Mensagem</h3>
  </div>
  <div className={styles.cardDescription}>
    Delete uma mensagem enviada. A mensagem será removida para todos os participantes da conversa.
  </div>
  <a href="/docs/messages/deletar" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="CheckCircle" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Marcar como Lida</h3>
  </div>
  <div className={styles.cardDescription}>
    Marque mensagens de um chat como lidas para remover indicadores de "não lido" e atualizar status.
  </div>
  <a href="/docs/messages/ler" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Reply" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Responder Mensagem</h3>
  </div>
  <div className={styles.cardDescription}>
    Responda a uma mensagem específica. A resposta aparecerá como uma resposta encadeada no WhatsApp.
  </div>
  <a href="/docs/messages/responder" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Pin" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Fixar/Desafixar</h3>
  </div>
  <div className={styles.cardDescription}>
    Fixe ou desafixe mensagens em um chat. Mensagens fixadas aparecem no topo e são destacadas.
  </div>
  <a href="/docs/messages/fixar-desafixar" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Zap" size="md" /> Novos Recursos

Explore as funcionalidades mais recentes adicionadas à API, incluindo novos tipos de mensagens interativas e eventos.

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MousePointerClick" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Botão OTP</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie códigos de autenticação (OTP) com botão de cópia fácil. Ótimo para 2FA.
  </div>
  <a href="/docs/messages/botao-otp" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="CreditCard" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Botão PIX</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie códigos PIX copia e cola com botão dedicado para facilitar pagamentos.
  </div>
  <a href="/docs/messages/botao-pix" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ShoppingBag" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Status do Pedido</h3>
  </div>
  <div className={styles.cardDescription}>
    Atualize o status de pedidos com mensagens estruturadas para seus clientes.
  </div>
  <a href="/docs/messages/atualizacao-status-pedido" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="UserPlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Convite Admin Canal</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie convites para adicionar novos administradores aos seus canais (newsletter).
  </div>
  <a href="/docs/messages/convite-admin-canal" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Calendar" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Enviar Evento</h3>
  </div>
  <div className={styles.cardDescription}>
    Envie convites para eventos com data, hora, localização e opção de chamada de vídeo.
  </div>
  <a href="/docs/messages/evento" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Edit" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Editar Evento</h3>
  </div>
  <div className={styles.cardDescription}>
    Edite informações de eventos já enviados, notificando os participantes.
  </div>
  <a href="/docs/messages/editar-evento" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="MessageCircle" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Responder Evento</h3>
  </div>
  <div className={styles.cardDescription}>
    Responda a convites de eventos via API (Comparecer, Talvez, Recusar).
  </div>
  <a href="/docs/messages/responder-evento" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

:::tip Próximos Passos

Comece pela [Visão Geral sobre Mensagens](/docs/messages/introducao) para entender os conceitos fundamentais que se aplicam a todos os tipos de mensagem.

:::
