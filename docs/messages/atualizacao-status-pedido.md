...
# <Icon name="RefreshCw" size="lg" /> Atualizar Status do Pedido

Atualize o status de um pedido enviado anteriormente. Use este método para notificar o cliente sobre mudanças no status do pedido (pending, processing, shipped, completed, canceled).
...
- **Processamento**: Notificar quando o pedido está sendo processado
- **Envio**: Informar quando o pedido foi enviado
- **Concluído**: Confirmar quando o pedido foi entregue
- **Cancelado**: Notificar sobre cancelamento do pedido

![Exemplo de atualização de status do pedido](/img/order-status-update.jpeg)

---

## <Icon name="Wand2" size="md" /> Para Usuários No-Code

Para atualizar o status de um pedido, você precisará dos seguintes dados retornados no webhook quando o pedido foi enviado:
...
4. **`paymentStatus`**: Status atual do pagamento
...
| `paymentStatus` | string | Status atual do pagamento: `pending` ou `paid` (retornado no webhook) |
...
É necessário enviar na requisição **todos os dados previamente informados** no envio do pedido, com a inclusão de alguns parâmetros adicionais obrigatórios, os quais são retornados no [webhook](/docs/webhooks/ao-receber#exemplo-de-retorno-de-atualização-de-pedido).
...
Quando o status do pedido é atualizado, o webhook [Ao receber mensagem](/docs/webhooks/ao-receber) será acionado com informações sobre a atualização.
...
- **Status do pagamento**: Use o `paymentStatus` retornado no webhook, não altere este valor aqui
...
- [Atualizar Pagamento do Pedido](/docs/messages/atualizacao-pagamento-pedido) - Atualize o status do pagamento
