...
# <Icon name="BookOpen" size="lg" /> Enviar catálogo

Envie um link para o seu catálogo WhatsApp Business completo para um destinatário usando a API do Z-API. O catálogo permite que os usuários naveguem por todos os seus produtos de forma organizada.

**⚠️ Importante:** Este recurso está disponível apenas para contas **business** do WhatsApp. É necessário também que a conta possua produtos e um catálogo configurado. Operações relacionadas a produtos e coleções podem ser encontradas na seção [WhatsApp Business](../whatsapp-business/introducao) da nossa documentação.

![Exemplo de mensagem de catálogo](/img/catalog-message.jpeg)
...
| `catalogPhone` | string | Sim | Telefone da conta business à qual pertence o catálogo. Deve ser o número da conta WhatsApp Business que possui o catálogo configurado. |

#### Parâmetros Opcionais

| Campo | Tipo | Obrigatório | Descrição |
|---------|--------|-------------|--------------------------------------------------|
| `translation` | string | Não | Idioma da mensagem padrão do card do catálogo. Valores possíveis: `"EN"` (inglês) ou `"PT"` (português). Se não informado, usa o idioma padrão da conta. |
| `message` | string | Não | Mensagem enviada com o card do catálogo. Esta mensagem aparece antes do card do catálogo. |
| `title` | string | Não | Título do card do catálogo. Aparece no card do catálogo enviado. |
| `catalogDescription` | string | Não | Descrição enviada no card do catálogo. Aparece abaixo do título no card do catálogo. |

## <Icon name="Info" size="md" /> Pré-requisitos {#pre-requisitos}

Para enviar catálogos, você precisa:
...
4. **Catálogo publicado** e acessível
5. **Conhecer o número da conta business** (`catalogPhone`) que possui o catálogo

:::tip Dica

Consulte a seção [WhatsApp Business](../whatsapp-business/introducao) para aprender como configurar e gerenciar seu catálogo, produtos e coleções.
...
1. **`phone`**: O número do contato que receberá o link do catálogo. Use o formato completo: DDI + DDD + Número (ex: `5511999999999`).

2. **`catalogPhone`**: O telefone da conta WhatsApp Business que possui o catálogo. **Importante:** Este deve ser o número da conta business onde o catálogo está configurado. Use o formato completo: DDI + DDD + Número (ex: `5511999999999`).

### Campos Opcionais

3. **`translation`**: O idioma do card do catálogo. Use `"PT"` para português ou `"EN"` para inglês. Se não informado, usa o idioma padrão da conta.

4. **`message`**: Uma mensagem que será enviada junto com o card do catálogo. Esta mensagem aparece antes do card. Exemplo: `"Acesse esse link para visualizar nosso catálogo no Whatsapp:"`.

5. **`title`**: O título que aparecerá no card do catálogo. Exemplo: `"Veja o catálogo de produtos no Whatsapp."`.

6. **`catalogDescription`**: A descrição que aparecerá no card do catálogo, abaixo do título. Exemplo: `"Saiba mais sobre os produtos e serviços dessa empresa."`.

**Dica:** Na maioria dos casos, você só precisa preencher `phone` e `catalogPhone`. Os outros campos são opcionais e podem ser deixados em branco, mas melhoram a apresentação do catálogo.

**Casos de uso comuns:**

- **E-commerce:** Compartilhar catálogo completo de produtos
- **Serviços:** Enviar catálogo de serviços oferecidos
- **Restaurantes:** Compartilhar cardápio digital
- **Lojas físicas:** Enviar catálogo de produtos disponíveis
...
| 404 | Catálogo não encontrado | Verifique se o catálogo está configurado e ativo |
...
- ⚠️ **Validação:** Valide `phone` (apenas números, 10-15 dígitos) e `message` (opcional, máximo 4096 caracteres) antes de enviar
...
Quando você envia um catálogo:

1. **Link do catálogo** é enviado para o destinatário
2. **Mensagem personalizada** (se fornecida) aparece junto com o link
3. **Acesso ao catálogo** - O usuário pode clicar para abrir e navegar por todos os produtos
4. **Navegação** - O usuário pode ver produtos organizados por categorias, preços e descrições

:::tip Boa prática

Use mensagens personalizadas para destacar ofertas especiais, novos produtos ou categorias específicas do seu catálogo.

:::

## <Icon name="AlertTriangle" size="md" /> Limitações {#limitacoes}

- Você precisa ter um catálogo WhatsApp Business configurado e ativo
- O catálogo deve ter produtos cadastrados para ser útil
- O catálogo precisa estar publicado e acessível

## <Icon name="Info" size="md" /> Notas importantes {#notas-importantes}

- Catálogos são ideais para e-commerce e vendas através do WhatsApp
- Use mensagens personalizadas para destacar características do seu catálogo
- Mantenha o catálogo atualizado com produtos, preços e imagens
- Catálogos permitem que clientes naveguem e escolham produtos de forma autônoma

## <Icon name="ArrowRight" size="md" /> Próximos passos {#proximos-passos}

- [Enviar produto](/docs/messages/produto) - Envie produtos individuais do catálogo
- [Gerenciar produtos](/docs/whatsapp-business/produtos) - Crie e gerencie produtos no catálogo
- [WhatsApp Business](/docs/whatsapp-business/introducao) - Saiba mais sobre funcionalidades Business
