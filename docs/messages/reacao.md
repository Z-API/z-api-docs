...
# <Icon name="Heart" size="lg" /> Adicionar reação

Adicione uma reação emoji a uma mensagem existente usando a API do Z-API. Reações permitem que você responda rapidamente a mensagens com emojis.
...
![Exemplo de reação](/img/send-message-reaction.jpeg)
...
| `reaction` | string | Sim | Emoji de reação. Veja opções de emoji neste [link](https://fsymbols.com/pt/emoji/) |
| `messageId` | string | Sim | ID da mensagem que vai receber a reação |
...
## <Icon name="Smile" size="md" /> Reações disponíveis {#reacoes-disponiveis}

Você pode usar qualquer emoji como reação. Os mais comuns incluem:

| Emoji | Nome | Uso comum |
|-------|------|-----------|
| 👍 | Joinha | Aprovação, concordância |
| ❤️ | Coração | Gostei, amor |
| 😂 | Riso | Engraçado, divertido |
| 😮 | Surpresa | Surpresa, espanto |
| 😢 | Triste | Tristeza, empatia |
| 🙏 | Oração | Gratidão, respeito |

:::tip Dica

Você pode usar qualquer emoji Unicode como reação. Explore diferentes emojis para expressar suas reações de forma mais precisa. Veja opções de emoji neste [link](https://fsymbols.com/pt/emoji/).
...
2. **`reaction`**: O emoji que você deseja usar como reação (ex: `"👍"`, `"❤️"`, `"😂"`). Você pode usar qualquer emoji Unicode. Veja opções de emoji neste [link](https://fsymbols.com/pt/emoji/).

3. **`messageId`**: O ID da mensagem que vai receber a reação. Este é o `messageId` da mensagem original que você deseja reagir. Você pode obter o `messageId` através dos webhooks quando uma mensagem é recebida ou enviada.

### Campos Opcionais

4. **`delayMessage`**: Delay em segundos (1-15 segundos) antes de enviar a reação. Se não informado, o delay padrão é de 1-3 segundos.
...
- **Obter messageId**: O `messageId` pode ser obtido através dos webhooks quando uma mensagem é recebida ou enviada. Guarde este ID se você quiser reagir a uma mensagem posteriormente.
- **Emojis válidos**: Use emojis Unicode válidos. Você pode copiar emojis de sites como [fsymbols.com](https://fsymbols.com/pt/emoji/) ou usar emojis diretamente do seu teclado.
- **Reações em grupos**: Você pode reagir a mensagens em grupos usando o ID do grupo no campo `phone`.
- **Delay opcional**: Use `delayMessage` se você quiser adicionar um delay antes de enviar a reação (útil para simular comportamento humano).

**Casos de uso comuns:**

- **Feedback rápido**: Reagir a mensagens de clientes com 👍 para confirmar recebimento
- **Aprovação**: Usar ❤️ para mostrar que gostou de algo
- **Empatia**: Usar 😢 ou 🙏 para expressar empatia em situações difíceis
- **Diversão**: Usar 😂 para reagir a mensagens engraçadas
- **Automação de atendimento**: Reagir automaticamente a mensagens recebidas para confirmar que foram lidas
...
Para saber quando a reação foi aplicada ou se houve algum erro, configure um webhook e monitore os eventos. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber#exemplo-de-retorno-de-reação).

### Erros comuns {#erros-comuns}

| Código | Motivo | Como resolver |
|--------|-----------------------|----------------------------------------------------|
| 400 | Parâmetros inválidos | Verifique `phone`, `messageId` e `reaction` |
...
- Você só pode adicionar uma reação por mensagem
- Se você já reagiu, adicionar uma nova reação substitui a anterior
- A mensagem original deve existir e ser acessível

:::info Comportamento

- Use o `messageId` da mensagem à qual deseja reagir
- O `phone` deve ser o número do remetente da mensagem original
- Reações aparecem abaixo da mensagem no WhatsApp
- Você pode remover uma reação usando o endpoint de [remover reação](/docs/messages/remover-reacao)

:::

## <Icon name="Info" size="md" /> Notas importantes {#notas-importantes}

- Reações são uma forma rápida e visual de responder a mensagens
- Use emojis apropriados para o contexto da mensagem
- Reações aparecem em tempo real para o remetente da mensagem original

## <Icon name="ArrowRight" size="md" /> Próximos passos {#proximos-passos}

- [Remover reação](/docs/messages/remover-reacao) - Remova uma reação de uma mensagem
