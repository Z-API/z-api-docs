...
Vídeos são uma forma poderosa de engajar seu público, seja para demonstrar um produto, contar uma história ou oferecer suporte visual. Com o Z-API, você pode enviar vídeos diretamente na conversa do WhatsApp, com a opção de adicionar uma legenda.

---

## <Icon name="Lightbulb" size="md" /> Casos de Uso Comuns

- **Demonstração de Produto:** Enviar um vídeo curto mostrando como um produto funciona.
- **Tutoriais Rápidos:** Criar guias visuais para ajudar clientes a resolver problemas comuns.
- **Mensagens de Boas-vindas:** Enviar uma mensagem de vídeo personalizada para novos clientes.
- **Marketing de Conteúdo:** Compartilhar conteúdo de vídeo relevante para sua audiência.

---

## <Icon name="HelpCircle" size="md" /> URL vs. Base64: Qual Usar?

Assim como nas imagens, você pode enviar vídeos de duas formas:

1. **URL (Altamente Recomendado):**
 - **O que é?** Um link direto para um vídeo hospedado na internet (ex: `https://meusite.com/video.mp4`).
 - **Quando usar?** É a forma mais eficiente e confiável. O vídeo precisa estar em um link público para que o Z-API possa acessá-lo.

2. **Base64:**
 - **O que é?** Uma codificação do arquivo de vídeo em formato de texto.
 - **Quando usar?** Apenas em situações específicas onde o vídeo é gerado dinamicamente e não está acessível por uma URL.

:::danger Atenção
O uso de Base64 para vídeos é fortemente desencorajado. Arquivos de vídeo são grandes, e a codificação em Base64 pode aumentar o tamanho em mais de 30%, levando a erros e falhas no envio. **Use URL sempre que possível.**
:::

![Exemplo de mensagem com vídeo](/img/send-message-video.jpeg)

---

## <Icon name="Wand2" size="md" /> Para Usuários No-Code

Em sua ferramenta de automação, a configuração para enviar um vídeo é simples:

### Campos Obrigatórios

1. **`phone`**: O número do contato que receberá o vídeo. Use o formato completo: DDI + DDD + Número (ex: `5511999999999`).
2. **`video`**: O campo onde você colará a **URL pública** do seu vídeo. A URL deve estar acessível publicamente na internet.

### Campos Opcionais

3. **`caption`**: O texto que servirá como legenda para o vídeo. Você pode usar formatação (negrito com `*texto*`, itálico com `_texto_`).
...
5. **`delayMessage`**: Se você vai enviar vários vídeos seguidas, use este campo para espaçar o envio (entre 1 e 15 segundos). Isso ajuda a evitar bloqueios e torna a comunicação mais natural.

6. **`viewOnce`**: Marque como `true` se quiser que o vídeo desapareça após ser visualizado (vídeo que some). Útil para vídeos sensíveis ou temporários.

7. **`async`**: Marque como `true` se o vídeo for muito grande e você quiser que a requisição responda imediatamente, processando o envio em segundo plano. O status do envio será notificado via webhook.

**Dica:** Na maioria dos casos, você só precisa preencher `phone` e `video`. Os outros campos são opcionais e podem ser deixados em branco.

---

## <Icon name="Code" size="md" /> Para Desenvolvedores

Para enviar um vídeo, faça uma requisição `POST` para o endpoint abaixo, fornecendo a URL pública do arquivo.
...
| `video` | string | Sim | A URL pública do vídeo. O uso de Base64 é tecnicamente possível, mas não recomendado. |

#### Parâmetros Opcionais

| Campo | Tipo | Obrigatório | Descrição |
|:-------- |:----- |:---------- |:----------------------------------------------------------------------- |
| `caption` | string | Não | O texto que acompanhará o vídeo. Permite formatação. Máx. 1024 caracteres. |
...
| `viewOnce` | boolean | Não | Define se o vídeo será uma mensagem de visualização única (vídeo que desaparece após ser visto). Padrão: `false` (vídeo normal, permanece no chat). Uma vez visualizado, o vídeo não pode ser visto novamente pelo destinatário. |
| `async` | boolean | Não | Se ativo (`true`), a requisição responderá imediatamente com sucesso e o processamento do arquivo será realizado em segundo plano. O envio pode ser verificado através do [webhook de envio](../webhooks/ao-enviar). Útil para vídeos grandes que podem demorar para processar. |

---

## <Icon name="Shield" size="md" /> Boas Práticas e Limites

- **Tamanho Máximo:** O WhatsApp limita o envio de vídeos a **64MB**.
- **Formatos Suportados:** MP4, 3GP, AVI, MOV. O formato **MP4 com codec H.264** é o mais recomendado para máxima compatibilidade.
- **Otimização:** Para uma experiência de envio e recebimento fluida, mantenha seus vídeos abaixo de **16MB** e com duração de até **3 minutos**.
- **Compressão:** O WhatsApp pode comprimir os vídeos automaticamente para otimizar a entrega.
...
- ⚠️ **Validação:** Valide `phone` (apenas números, 10-15 dígitos), `video` (URL válida HTTP/HTTPS) e `caption` (máximo 1024 caracteres) antes de enviar
...
Se você usou o parâmetro `async: true`, a resposta será imediata, mas o processamento do vídeo acontecerá em segundo plano. O status do envio será notificado através do [webhook de envio](../webhooks/ao-enviar).

Para uma lista completa de possíveis erros, como arquivo muito grande ou formato não suportado, consulte a seção **[Erros Comuns](../instance/status#erros-comuns)**.
