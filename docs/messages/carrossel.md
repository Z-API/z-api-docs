...
# <Icon name="Layers" size="lg" /> Enviar Carrossel

Envie mensagens de carrossel com imagens, texto e botões de ação. Os carrosséis permitem apresentar múltiplos produtos ou opções de forma visual e interativa.

---

:::caution Atenção

Envios de botões que acompanham o carrossel atualmente se encontram disponíveis, porém possuem alguns fatores decisivos para o funcionamento. Para mais detalhes, acesse o tópico [Funcionamento dos Botões](/docs/tips/funcionamento-botoes).
...
- **Catálogo de Produtos**: Apresentar múltiplos produtos em um único carrossel
- **Menu de Opções**: Exibir várias opções de forma visual
- **Portfólio**: Mostrar trabalhos ou projetos em formato de carrossel
- **Promoções**: Destacar múltiplas ofertas simultaneamente
...
1. **`phone`**: Número do destinatário no formato completo: DDI + DDD + Número (ex: `5511999999999`). **Importante:** Use apenas números, sem formatação ou máscara.

2. **`message`**: Texto da mensagem principal que aparecerá antes do carrossel. Esta é a mensagem introdutória que contextualiza o carrossel.

3. **`carousel`**: Array (lista) de cartões do carrossel. Cada cartão é um objeto com:
   - **`text`** (obrigatório): Texto descritivo do cartão
   - **`image`** (obrigatório): URL pública da imagem do cartão (deve ser acessível via HTTPS)
   - **`buttons`** (opcional): Array de botões de ação.

### Configurando Botões nos Cartões

Cada botão é um objeto com os seguintes campos:

- **`type`** (obrigatório): Tipo do botão. Valores possíveis:
  - `"URL"`: Botão que abre um link
  - `"CALL"`: Botão que faz uma chamada telefônica
  - `"REPLY"`: Botão que envia uma resposta pré-definida (usado em chatbots)
- **`label`** (obrigatório): Texto que aparece no botão (ex: "Ver Produto", "Ligar Agora", "Sim")
- **`id`** (opcional): Identificador único do botão. Use este ID para identificar qual botão foi clicado quando receber o webhook
- **`url`** (obrigatório se `type` for `"URL"`): Link que será aberto quando o botão for clicado
- **`phone`** (obrigatório se `type` for `"CALL"`): Número de telefone que será chamado (formato: DDI + DDD + Número)

### Exemplo Prático para No-Code

**Exemplo 1: Carrossel simples (sem botões)**
...
**Exemplo 2: Carrossel com botões**
...
- **URLs das imagens**: Use URLs públicas e acessíveis (HTTPS recomendado). Imagens privadas ou que requerem autenticação não funcionarão
- **Identificadores de botões**: Use `id` únicos nos botões para identificar cliques no webhook
- **Tipos de botão**: 
  - `URL`: Abre um link no navegador
  - `CALL`: Inicia uma chamada telefônica
  - `REPLY`: Envia uma resposta pré-definida (útil para chatbots)
- **Delay opcional**: Você pode adicionar `delayMessage` (1-15 segundos) para controlar o tempo de envio

**Casos de uso comuns:**

- **E-commerce**: Mostrar múltiplos produtos com botões "Ver Produto" e "Comprar"
- **Restaurantes**: Exibir pratos do cardápio com botões "Fazer Pedido" e "Ver Cardápio Completo"
- **Serviços**: Apresentar diferentes serviços com botões "Solicitar Orçamento" e "Ligar Agora"
- **Portfólio**: Mostrar trabalhos realizados com botões "Ver Mais" e "Solicitar Proposta"
...
![Exemplo de mensagem carrossel](/img/send-message-carousel.jpeg)
...
| `carousel` | array | Array de objetos `carouselCard` com os cartões do carrossel |

### Estrutura do `carouselCard`

| Atributo | Tipo | Obrigatório | Descrição |
|----------|------|-------------|-----------|
| `text` | string | Sim | Texto do cartão |
| `image` | string | Sim | URL da imagem do cartão |
| `buttons` | array | Não | Array de objetos `buttonActions` |

### Estrutura do `buttonActions`

| Atributo | Tipo | Obrigatório | Descrição |
|----------|------|-------------|-----------|
| `type` | string | Sim | Tipo do botão: `CALL`, `URL`, ou `REPLY` |
| `label` | string | Sim | Texto do botão |
| `id` | string | Não | Identificador único do botão (usado no webhook) |
| `phone` | string | Não | Número para botões do tipo `CALL` |
| `url` | string | Não | Link para botões do tipo `URL` |
...
Para saber quando a mensagem foi entregue, lida, ou quando um botão foi clicado, configure um webhook e monitore os eventos. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber) e [webhooks de botões clicados](../webhooks/ao-receber#exemplo-de-retorno-de-carrosel).
...
[Webhook ao receber mensagem](/docs/webhooks/ao-receber) - Receba notificações quando botões do carrossel forem clicados

---

## <Icon name="Lightbulb" size="md" /> Dicas {#dicas}

- **Tipos de Botão**: Use `URL` para links, `CALL` para chamadas, e `REPLY` para respostas
- **Identificadores**: Use `id` nos botões para identificar qual foi clicado no webhook
- **Imagens**: Use URLs públicas e de boa qualidade para melhor visualização

---

## <Icon name="Rocket" size="md" /> Próximos Passos

- [Botões](/docs/messages/botoes) - Entenda como funcionam os botões
- [Funcionamento dos Botões](/docs/tips/funcionamento-botoes) - Status atual dos botões
