...
# <Icon name="Reply" size="lg" /> Responder mensagem

Responda a uma mensagem específica usando a API do Z-API. A resposta aparecerá como uma resposta encadeada no WhatsApp, vinculada à mensagem original.

Nesse tópico falaremos um pouco sobre como responder uma mensagem diretamente!

Quando você utiliza o método [send-text](./texto-simples) existe um atributo opcional chamado **messageId**, esse é um atributo que recebe o Id de uma mensagem qualquer. Quando esse atributo é passado, sua mensagem será diretamente relacionada à mensagem do Id informado.

:::tip Dica

Caso tenha qualquer dúvida sobre como enviar uma mensagem de texto, você pode ler sobre isso no nosso tópico de [**Enviar texto simples**](./texto-simples).

:::

![Exemplo de resposta a mensagem](/img/reply-message.jpeg)
...
**Importante:** Para responder uma mensagem, você usa o mesmo endpoint de enviar texto simples (`/send-text`), mas incluindo o parâmetro `messageId` da mensagem original.
...
**Exemplo para grupos com resposta privada:**
...
| `message` | string | Sim | Texto a ser enviado |
...
2. **`message`**: O texto da sua resposta. Você pode usar formatação Markdown (negrito com `*texto*`, itálico com `_texto_`, etc.).

3. **`messageId`**: O ID da mensagem original à qual você está respondendo. Este é o `messageId` da mensagem que você recebeu através dos webhooks ou que você enviou anteriormente (obtido no response da API).

### Campos Opcionais

1. **`delayMessage`**: Delay em segundos (1-15 segundos) antes de enviar a resposta. Se não informado, o delay padrão é de 1-3 segundos.

2. **`privateAnswer`**: Para mensagens em grupos, define se a resposta será enviada no grupo ou no particular do remetente. Use `true` para enviar no particular, `false` ou omita para enviar no grupo. **Importante:** Se o remetente for você mesmo, este parâmetro será ignorado e a resposta será enviada no grupo.
...
- **Obter messageId**: O `messageId` pode ser obtido através dos webhooks quando uma mensagem é recebida. Guarde este ID se você quiser responder posteriormente.
- **Respostas encadeadas**: Quando você inclui o `messageId`, a mensagem aparecerá como uma resposta encadeada no WhatsApp, vinculada à mensagem original.
- **Grupos**: Para responder em grupos, use o ID do grupo no campo `phone`. Use `privateAnswer: true` para responder no particular do remetente.
- **Formatação**: Você pode usar formatação Markdown na mensagem (negrito, itálico, etc.).

**Casos de uso comuns:**

- **Atendimento ao cliente**: Responder diretamente a perguntas de clientes
- **Suporte técnico**: Responder a dúvidas técnicas de forma organizada
- **Automação de respostas**: Criar respostas automáticas para mensagens recebidas
- **Gestão de grupos**: Responder mensagens em grupos de forma organizada
...
Para saber quando a mensagem foi entregue ou se houve algum erro, configure um webhook e monitore os eventos. Veja mais sobre [webhooks de mensagens recebidas](../webhooks/ao-receber#response).

### Erros comuns {#erros-comuns}

| Código | Motivo | Como resolver |
|--------|-----------------------|----------------------------------------------------|
| 400 | Parâmetros inválidos | Verifique `phone`, `messageId` e `message` |
...
Quando você responde a uma mensagem:

1. **A resposta aparece encadeada** - A mensagem de resposta aparece vinculada à mensagem original
2. **Preview da mensagem original** - O WhatsApp mostra um preview da mensagem original acima da sua resposta
3. **Contexto mantido** - Isso ajuda a manter o contexto da conversa, especialmente em grupos

:::tip Boa prática

Use respostas encadeadas para manter o contexto em conversas longas ou em grupos, facilitando a compreensão de qual mensagem você está respondendo.

:::

## <Icon name="AlertTriangle" size="md" /> Limitações {#limitacoes}

- Você só pode responder a mensagens que você recebeu ou enviou
- Mensagens deletadas não podem ser respondidas
- O `messageId` deve ser válido e acessível

## <Icon name="Info" size="md" /> Notas importantes {#notas-importantes}

- A resposta aparece como uma mensagem encadeada no WhatsApp, mostrando um preview da mensagem original
...
- Respostas ajudam a manter o contexto da conversa no WhatsApp
- Você pode responder a qualquer tipo de mensagem (texto, mídia, etc.)
...
- [Adicionar reação](/docs/messages/reacao) - Adicione reações emoji a mensagens
