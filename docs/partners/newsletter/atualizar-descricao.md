...
# <Icon name="FileText" size="lg" /> Atualizar Descrição do Canal

Altere a descrição de um canal de newsletter existente.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método é responsável por alterar a descrição de um canal já existente. A descrição do canal é exibida publicamente e ajuda os seguidores a entender o propósito do canal.
...
| `400` | Dados inválidos | Verifique se `id` e `description` foram fornecidos e se o ID contém o sufixo `@newsletter` |
...
- **Descrição pública**: A descrição é exibida publicamente no perfil do canal
- **Permissões**: Apenas proprietários e administradores podem alterar a descrição do canal
