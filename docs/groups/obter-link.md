...
# <Icon name="Link" size="lg" /> Obter Link de Convite

Obtenha o link de convite de um grupo através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite obter o link de convite de um grupo. O link pode ser compartilhado para permitir que outras pessoas entrem no grupo.
...
// Obter link com tratamento seguro de erros
async function getGroupInvitationLink() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/group-invitation-link/${encodeURIComponent(groupId)}`;
    ...
    if (result.invitationLink) {
      ...
      console.error('Erro ao obter link de convite');
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao obter link de convite:', error.message);
    throw error;
  }
}
...
interface GetGroupInvitationLinkResponse {
  phone: string;
...
// Função para obter link
async function getGroupInvitationLink(): Promise<GetGroupInvitationLinkResponse> {
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/group-invitation-link/{group_id}"
...
try:
    ...
    if result.get('invitationLink'):
        print('Link de convite obtido com sucesso')
    else:
...
# Obter link via cURL
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/group-invitation-link/${GROUP_ID}" \
...
// Obter link
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/group-invitation-link/${encodeURIComponent(groupId)}`);
...
// Rota para obter link
app.post('/group-invitation-link/:groupId', async (req, res) => {
  try {
    // Dados da requisição com validação
...
// Rota para obter link
router.post('/group-invitation-link/:groupId', async (ctx) => {
  try {
    // Dados da requisição com validação
...
app.on('error', (err, ctx) => {
  console.error('Erro ao obter link de convite:', err.message);
});
...
public class GetGroupInvitationLink {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/group-invitation-link/%s",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Link de convite obtido com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class GetGroupInvitationLink
{
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/group-invitation-link/{Uri.EscapeDataString(groupId)}";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Link de convite obtido com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/group-invitation-link/%s",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Link de convite obtido com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        if ($result['invitationLink'] ?? false) {
            echo "Link de convite obtido com sucesso\n";
        } else {
            echo "Erro ao obter link de convite\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/group-invitation-link/#{URI.encode_www_form_component(group_id)}")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['invitationLink']
      puts "Link de convite obtido com sucesso"
    else
      puts "Erro ao obter link de convite"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/group-invitation-link/\(groupId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")") else {
...
try {
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/group-invitation-link/$([System.Web.HttpUtility]::UrlEncode($groupId))"
    ...
    if ($response.invitationLink) {
        Write-Host "Link de convite obtido com sucesso"
    } else {
        Write-Host "Erro ao obter link de convite"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/group-invitation-link/120363019502650977-group HTTP/1.1
Host: api.z-api.io
Client-Token: SEU_CLIENT_TOKEN
Content-Type: application/json
```
...
int main() {
    try {
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + urlEncode(instanceId) + "/token/" + urlEncode(instanceToken) + "/group-invitation-link/" + urlEncode(groupId);
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Link de convite obtido com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    if (curl) {
        char url[1024];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/group-invitation-link/%s",
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Link de convite obtido com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
- **Link de convite**: O link retornado pode ser compartilhado para permitir que outras pessoas entrem no grupo
...
- **Redefinir link**: Se precisar gerar um novo link, use o método [Redefinir Link de Convite](/docs/groups/redefinir-link)
