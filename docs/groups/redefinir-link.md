...
Redefina o link de convite de um grupo, gerando um novo link através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite redefinir o link de convite de um grupo, gerando um novo link. O link antigo deixa de funcionar e um novo link é criado.
...
// Redefinir link com tratamento seguro de erros
async function redefineInvitationLink() {
  try {
    // ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/redefine-invitation-link/${encodeURIComponent(groupId)}`;
    ...
    if (result.invitationLink) {
      console.log('Link de convite redefinido com sucesso');
    } else {
      console.error('Erro ao redefinir link de convite');
    }
    return result;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro
    console.error('Erro ao redefinir link de convite:', error.message);
    throw error;
  }
}
...
interface RedefineInvitationLinkResponse {
...
// Função para redefinir link
async function redefineInvitationLink(): Promise<RedefineInvitationLinkResponse> {
...
redefineInvitationLink()
  .then((result) => console.log('Sucesso:', result.invitationLink))
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/redefine-invitation-link/{group_id}"
...
try:
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result.get('invitationLink'):
        print('Link de convite redefinido com sucesso')
    else:
        print('Erro ao redefinir link de convite')
...
# Redefinir link via cURL
curl -X POST \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/redefine-invitation-link/${GROUP_ID}" \
...
// Redefinir link
const url = new URL(`https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/redefine-invitation-link/${encodeURIComponent(groupId)}`);
...
// Rota para redefinir link
app.post('/redefine-invitation-link/:groupId', async (req, res) => {
  try {
    // Dados da requisição com validação
...
// Rota para redefinir link
router.post('/redefine-invitation-link/:groupId', async (ctx) => {
  try {
    // Dados da requisição com validação
...
app.on('error', (err, ctx) => {
  console.error('Erro ao redefinir link de convite:', err.message);
});
...
public class RedefineInvitationLink {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    public static void main(String[] args) {
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/redefine-invitation-link/%s",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                try (BufferedReader br = new BufferedReader(
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    System.out.println("Link de convite redefinido com sucesso");
                }
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
public class RedefineInvitationLink
{
    ...
    public static async Task Main(string[] args)
    {
        try
        {
            ...
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            string url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/redefine-invitation-link/{Uri.EscapeDataString(groupId)}";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Link de convite redefinido com sucesso");
                }
                else
                {
                    // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
func main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/redefine-invitation-link/%s",
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Printf("Link de convite redefinido com sucesso\n")
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
try {
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        if ($result['invitationLink'] ?? false) {
            echo "Link de convite redefinido com sucesso\n";
        } else {
            echo "Erro ao redefinir link de convite\n";
        }
    } else {
        // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
begin
  ...
  # ⚠️ SEGURANÇA: Sempre use HTTPS
  url = URI("https://api.z-api.io/instances/#{URI.encode_www_form_component(INSTANCE_ID)}/token/#{URI.encode_www_form_component(INSTANCE_TOKEN)}/redefine-invitation-link/#{URI.encode_www_form_component(group_id)}")
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    if result['invitationLink']
      puts "Link de convite redefinido com sucesso"
    else
      puts "Erro ao redefinir link de convite"
    end
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")/redefine-invitation-link/\(groupId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")") else {
    ...
    if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
        if let data = data {
            do {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                if let invitationLink = result?["invitationLink"] as? String, !invitationLink.isEmpty {
                    print("Link de convite redefinido com sucesso")
                } else {
                    print("Erro ao redefinir link de convite")
                }
...
try {
    ...
    # ⚠️ SEGURANÇA: Sempre use HTTPS
    $url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/redefine-invitation-link/$([System.Web.HttpUtility]::UrlEncode($groupId))"
    ...
    if ($response.invitationLink) {
        Write-Host "Link de convite redefinido com sucesso"
    } else {
        Write-Host "Erro ao redefinir link de convite"
    }
} catch {
    # ⚠️ SEGURANÇA: Tratamento genérico de erro
...
```http
POST https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/redefine-invitation-link/120363019502650977-group HTTP/1.1
Host: api.z-api.io
Client-Token: SEU_CLIENT_TOKEN
Content-Type: application/json
```
...
int main() {
    try {
        ...
        if (curl) {
            std::string url = "https://api.z-api.io/instances/" + urlEncode(instanceId) + "/token/" + urlEncode(instanceToken) + "/redefine-invitation-link/" + urlEncode(groupId);
            ...
            if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Link de convite redefinido com sucesso" << std::endl;
            } else {
                // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
int main() {
    ...
    char encodedInstanceId[256];
    char encodedInstanceToken[256];
    char encodedGroupId[256];
    ...
    urlEncode(instanceId, encodedInstanceId, sizeof(encodedInstanceId));
    urlEncode(instanceToken, encodedInstanceToken, sizeof(encodedInstanceToken));
    urlEncode(cleanedGroupId, encodedGroupId, sizeof(encodedGroupId));

    CURL* curl = curl_easy_init();
    if (curl) {
        char url[1024];
        snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/redefine-invitation-link/%s",
                 encodedInstanceId, encodedInstanceToken, encodedGroupId);
        ...
        if (res == CURLE_OK && responseCode >= 200 && responseCode < 300) {
            // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
            printf("Link de convite redefinido com sucesso\n");
        } else {
            // ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
|-------|------|-----------|
| `invitationLink` | string | Novo link de convite do grupo |
...
- **Link antigo**: O link anterior deixa de funcionar após a redefinição
- **Novo link**: Um novo link é gerado e retornado na resposta
...
- **Uso**: Use este método quando precisar invalidar um link antigo e gerar um novo
