...
# <Icon name="FileText" size="lg" /> Alterar Descrição do Grupo

Altere a descrição de um grupo existente através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite alterar a descrição de um grupo. A descrição é exibida na página de informações do grupo e pode ser visualizada por todos os participantes.
...
| `groupDescription` | string | Nova descrição do grupo |
...
| `400` | Parâmetros inválidos | Verifique se o `groupId` e `groupDescription` estão corretos |
...
- **Permissão**: Apenas administradores do grupo podem alterar a descrição
- **Visibilidade**: A descrição é visível para todos os participantes do grupo
