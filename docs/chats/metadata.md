...
# <Icon name="Info" size="lg" /> Metadata do Chat

Obtenha informações detalhadas sobre um chat através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método retorna todas as informações de metadata de um chat, incluindo número de mensagens não lidas, última interação, status de silenciamento, spam, foto do perfil, anotações (WhatsApp Business) e outras informações relevantes.
...
| `unread` | string | Número de mensagens não lidas no chat |
| `lastMessageTime` | string | Timestamp com a data e hora da última interação com o chat |
| `isMuted` | string | `"0"` ou `"1"` - indica se você silenciou este chat |
...
| `notes` | object | Anotações atribuídas ao chat (somente WhatsApp Business) |
| `notes.id` | string | ID da anotação |
| `notes.content` | string | Texto da anotação |
| `notes.createdAt` | number | Timestamp da criação da anotação |
| `notes.lastUpdateAt` | number | Timestamp da última atualização da anotação |
| `messagesUnread` | integer | ** Descontinuado** - use `unread` |
...
| `404` | Chat não encontrado | Verifique se o número informado existe e se há histórico de conversa |
...
- **Foto do perfil**: A URL `profileThumbnail` expira após 48 horas
- **Anotações**: O campo `notes` só está disponível para contas WhatsApp Business
- **Grupos**: Use `isGroup` para identificar se o chat é um grupo ou conversa individual
- **Grupos de avisos**: `isGroupAnnouncement` indica se é um grupo onde apenas administradores podem enviar mensagens
- **Mensagens não lidas**: Use o campo `unread` (string) em vez de `messagesUnread` (descontinuado)
- **Spam**: Use `isMarkedSpam` para verificar se o chat foi marcado como spam
- **Silenciado**: Use `isMuted` para verificar se o chat está silenciado (`"1"` = silenciado, `"0"` = não silenciado)

---

## <Icon name="Link" size="md" /> Métodos Relacionados {#relacionados}

- [Listar Chats](/docs/chats/pegar) - Liste todos os chats
- [Ler Chat](/docs/chats/ler) - Marque mensagens como lidas
- [Arquivar Chat](/docs/chats/arquivar) - Arquive ou desarquive um chat
- [Fixar Chat](/docs/chats/fixar) - Fixe ou desafixe um chat
- [Silenciar Chat](/docs/chats/mutar) - Silencie ou desmute um chat
- [Limpar Chat](/docs/chats/limpar) - Limpe o histórico de mensagens
- [Deletar Chat](/docs/chats/deletar) - Delete um chat permanentemente
