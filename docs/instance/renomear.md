...
# <Icon name="Edit" size="lg" /> Renomear Instância

Renomeie uma instância através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite renomear uma instância existente. O nome da instância é usado apenas para identificação no painel administrativo e não afeta o funcionamento da API.
...
|-------|------|-----------|
| `value` | boolean | `true` se a renomeação foi bem-sucedida, `false` em caso de falha |
...
- O nome da instância é usado apenas para **identificação no painel administrativo**
- A renomeação não afeta o funcionamento da API ou a conexão com o WhatsApp
- O novo nome será aplicado imediatamente após a requisição bem-sucedida
- Não há limite de caracteres, mas recomenda-se manter o nome conciso e descritivo
