---
title: Gerenciando a Instância
sidebar_position: 0
description: Aprenda a conectar, monitorar e configurar sua instância do WhatsApp.
---

import styles from './index.module.css';

## <Icon name="Smartphone" size="lg" /> Gerenciando a Instância

<div className={styles.heroSection}>

A instância é o coração da sua conexão com o Z-API. É a ponte digital que conecta um número de WhatsApp à nossa plataforma, permitindo que suas automações enviem e recebam mensagens.

</div>

---

## <Icon name="QrCode" size="md" /> Conexão e Autenticação

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Smartphone" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Gerenciando sua Instância</h3>
  </div>
  <div className={styles.cardDescription}>
    Entenda o conceito de instância e como ela funciona como ponte entre seu WhatsApp e a plataforma Z-API.
  </div>
  <a href="/docs/instance/introducao" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="QrCode" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>QR Code</h3>
  </div>
  <div className={styles.cardDescription}>
    Conecte sua instância escaneando o QR Code gerado pelo Z-API usando o WhatsApp do seu celular.
  </div>
  <a href="/docs/instance/qrcode" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Circle" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Status</h3>
  </div>
  <div className={styles.cardDescription}>
    Verifique o status da sua instância para saber se está conectada, desconectada ou em processo de conexão.
  </div>
  <a href="/docs/instance/status" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="RotateCcw" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Reiniciar</h3>
  </div>
  <div className={styles.cardDescription}>
    Reinicie sua instância quando necessário para resolver problemas de conexão ou atualizar configurações.
  </div>
  <a href="/docs/instance/reiniciar" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="LogOut" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Desconectar</h3>
  </div>
  <div className={styles.cardDescription}>
    Desconecte sua instância de forma segura quando precisar encerrar a conexão temporariamente.
  </div>
  <a href="/docs/instance/desconectar" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Eye" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Leitura Automática</h3>
  </div>
  <div className={styles.cardDescription}>
    Configure a leitura automática de mensagens para marcar mensagens recebidas como lidas automaticamente.
  </div>
  <a href="/docs/instance/leitura-automatica" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="PhoneOff" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Rejeitar Chamadas</h3>
  </div>
  <div className={styles.cardDescription}>
    Configure sua instância para rejeitar automaticamente chamadas de voz e vídeo recebidas.
  </div>
  <a href="/docs/instance/rejeitar-chamadas" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="UserCircle" size="md" /> Configurações do Perfil

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="ImagePlus" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Atualizar Imagem do Perfil</h3>
  </div>
  <div className={styles.cardDescription}>
    Atualize a foto de perfil da sua instância do WhatsApp através da API do Z-API.
  </div>
  <a href="/docs/instance/atualizar-imagem-perfil" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Edit3" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Atualizar Nome do Perfil</h3>
  </div>
  <div className={styles.cardDescription}>
    Altere o nome exibido no perfil da sua instância do WhatsApp.
  </div>
  <a href="/docs/instance/atualizar-nome-perfil" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="FileText" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Atualizar Descrição do Perfil</h3>
  </div>
  <div className={styles.cardDescription}>
    Atualize a descrição (status) do perfil da sua instância do WhatsApp.
  </div>
  <a href="/docs/instance/atualizar-descricao-perfil" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

## <Icon name="Database" size="md" /> Dados da Instância

<div className={styles.cardsGrid}>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Database" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Dados</h3>
  </div>
  <div className={styles.cardDescription}>
    Obtenha informações completas sobre sua instância, incluindo status, configurações e metadados.
  </div>
  <a href="/docs/instance/dados" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Smartphone" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Dados do Celular</h3>
  </div>
  <div className={styles.cardDescription}>
    Obtenha informações sobre o dispositivo celular associado à sua instância.
  </div>
  <a href="/docs/instance/dados-celular" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

<div className={styles.card}>
  <div className={styles.cardHeader}>
    <Icon name="Edit3" size="md" className={styles.cardIcon} />
    <h3 className={styles.cardTitle}>Renomear</h3>
  </div>
  <div className={styles.cardDescription}>
    Renomeie sua instância para facilitar a identificação e organização no painel do Z-API.
  </div>
  <a href="/docs/instance/renomear" className={styles.cardLink}>
    Ver guia →
  </a>
</div>

</div>

---

:::tip Próximos Passos

Depois de configurar sua instância, explore a seção de [Mensagens](/docs/messages/introducao) para começar a enviar conteúdo.

:::
