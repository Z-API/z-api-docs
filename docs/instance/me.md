...
# <Icon name="Info" size="lg" /> Dados da instância

Obtenha todas as informações sobre sua instância através da API do Z-API.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método te permite obter os dados completos da sua instância, incluindo informações de conexão, configurações de webhooks, status de pagamento e outras configurações importantes.
...
- Este endpoint retorna todas as informações da instância em um único request
- O campo `connected` indica se a instância está atualmente conectada ao WhatsApp
- Os campos de webhook podem ser `null` se não estiverem configurados
- O campo `due` contém um timestamp Unix que representa a data de vencimento da instância
- Use este endpoint para verificar o status e configurações da sua instância antes de realizar operações
