---
id: dados
title: Obter dados da instância
sidebar_position: 1
---
...
O endpoint de obtenção de dados da instância é uma das operações mais fundamentais da API do Z-API. Ele permite recuperar informações completas e atualizadas sobre sua instância, incluindo o status atual de conexão com o WhatsApp, todas as configurações ativas, detalhes técnicos do dispositivo conectado e configurações de webhooks.

**Por que este endpoint é importante?**

Este endpoint é essencial para monitoramento e diagnóstico. Antes de enviar mensagens, você deve verificar se a instância está conectada (`status: "connected"`). Ele também fornece informações valiosas sobre o dispositivo WhatsApp conectado (versão do aplicativo, sistema operacional, modelo do dispositivo), o que pode ser útil para troubleshooting e para entender o ambiente de execução.

**Quando usar este endpoint:**

- **Verificação de Status**: Antes de enviar mensagens, verifique se a instância está conectada
- **Monitoramento**: Implemente verificações periódicas para monitorar a saúde da instância
- **Diagnóstico**: Use as informações do dispositivo para diagnosticar problemas de compatibilidade
- **Configuração**: Verifique as configurações atuais de webhooks e outras opções
...
| status | string | Status atual da instância (`connected`, `disconnected`, `qrcode`) |
| qrcode | string | QR Code (se disponível, caso contrário `null`) |
| phone | object | Informações do dispositivo conectado |
| webhook | object | Configurações de webhook |
...
O campo `status` pode ter três valores principais:

- **`"connected"`**: A instância está conectada e pronta para enviar e receber mensagens. Este é o estado ideal para operação normal.
- **`"disconnected"`**: A instância está desconectada do WhatsApp. Você precisará reconectar escaneando um novo QR Code.
- **`"qrcode"`**: A instância está aguardando autenticação. Um QR Code está disponível no campo `qrcode` para ser escaneado.

### QR Code para Autenticação

O campo `qrcode` contém uma string base64 com a imagem do código QR quando a instância está desconectada ou aguardando autenticação. Quando a instância já está conectada, este campo retorna `null`. 

**Como usar o QR Code:**
1. Decodifique a string base64 para obter a imagem
2. Exiba a imagem para o usuário escanear com o WhatsApp
3. Após o escaneamento, o status mudará para `"connected"`

### Informações do Dispositivo

O objeto `phone` contém informações técnicas valiosas sobre o dispositivo WhatsApp conectado:

- **`wa_version`**: Versão do WhatsApp instalada no dispositivo. Útil para verificar compatibilidade com recursos específicos.
- **`os_version`**: Sistema operacional e versão (ex: "Android 13", "iOS 17.0"). Ajuda no diagnóstico de problemas específicos de plataforma.
- **`device_manufacturer`** e **`device_model`**: Fabricante e modelo do dispositivo. Útil para suporte técnico e troubleshooting.

**Importante**: Essas informações são atualizadas automaticamente quando a instância se conecta ao WhatsApp. Se você não vir essas informações, a instância pode não estar completamente conectada.

### Configurações de Webhook

O objeto `webhook` exibe todas as configurações ativas de webhooks da sua instância:

- **`url`**: A URL configurada para receber eventos em tempo real
- **`events`**: Array com os tipos de eventos configurados (ex: `["message", "status"]`)

Use essas informações para verificar se seus webhooks estão configurados corretamente e quais eventos estão sendo monitorados.

## Casos de Uso Práticos

### Verificação Antes de Enviar Mensagens

Sempre verifique o status da instância antes de tentar enviar mensagens para evitar erros:
...
Implemente verificações periódicas para monitorar a saúde da instância:
...
Use as informações do dispositivo para diagnosticar problemas:
