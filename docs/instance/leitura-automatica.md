...
# <Icon name="CheckCircle" size="lg" /> Leitura Automática

Ative a leitura automática de todas as mensagens recebidas pela sua instância. Quando ativada, as mensagens serão marcadas como lidas automaticamente.

---

## <Icon name="Info" size="md" /> Conceituação {#conceituacao}

Este método permite configurar a leitura automática de mensagens. Quando ativado, todas as mensagens recebidas pela API serão marcadas como lidas automaticamente, sem necessidade de intervenção manual.
...
|----------|------|-----------|
| `value` | boolean | `true` para ativar leitura automática, `false` para desativar |
...
// Ativar/desativar leitura automática com validação
async function updateAutoReadMessage(enabled) {
  try {
    ...
    const url = `https://api.z-api.io/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/update-auto-read-message`;
    ...
    console.log(`Leitura automática ${enabled ? 'ativada' : 'desativada'} com sucesso`);
    return data;
  } catch (error) {
    // ⚠️ SEGURANÇA: Tratamento genérico de erro sem expor stack traces em produção
    console.error('Erro ao atualizar leitura automática:', error.message);
    throw error;
  }
}

// Exemplo de uso: ativar leitura automática
updateAutoReadMessage(true);
```
...
interface AutoReadResponse {
  value: boolean;
}

// Função para atualizar leitura automática
async function updateAutoReadMessage(enabled: boolean): Promise<AutoReadResponse> {
...
updateAutoReadMessage(true)
  .then((result) => {
    console.log(`Leitura automática ${result.value ? 'ativada' : 'desativada'} com sucesso`);
  })
...
url = f"https://api.z-api.io/instances/{INSTANCE_ID}/token/{INSTANCE_TOKEN}/update-auto-read-message"
...
# Atualizar leitura automática com validação
def update_auto_read_message(enabled: bool) -> Dict[str, Any]:
    # ⚠️ VALIDAÇÃO: Garantir que o valor é boolean
    ...
    try:
        ...
        print(f'Leitura automática {status} com sucesso')
        return result
...
# ⚠️ SEGURANÇA: Sempre use HTTPS (nunca HTTP)
# Atualizar leitura automática via cURL
curl -X PUT \
  "https://api.z-api.io/instances/${INSTANCE_ID}/token/${INSTANCE_TOKEN}/update-auto-read-message" \
...
function updateAutoReadMessage(enabled) {
  return new Promise((resolve, reject) => {
    ...
    const path = `/instances/${encodeURIComponent(instanceId)}/token/${encodeURIComponent(instanceToken)}/update-auto-read-message`;
...
// Rota para atualizar leitura automática
app.put('/api/auto-read', async (req, res) => {
  try {
    ...
    res.status(error.response?.status || 500).json({
      success: false,
      error: 'Erro ao atualizar leitura automática',
    });
  }
});
...
// Middleware para atualizar leitura automática
app.use(async (ctx) => {
  if (ctx.path === '/api/auto-read' && ctx.method === 'PUT') {
    try {
      // ⚠️ VALIDAÇÃO: Validar entrada do usuário
...
public class UpdateAutoReadMessage {
    // ⚠️ SEGURANÇA: Use variáveis de ambiente para credenciais
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String urlString = String.format(
                "https://api.z-api.io/instances/%s/token/%s/update-auto-read-message",
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                System.out.println("Leitura automática atualizada com sucesso");
...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        {
            // ⚠️ SEGURANÇA: Sempre use HTTPS
            var url = $"https://api.z-api.io/instances/{Uri.EscapeDataString(InstanceId)}/token/{Uri.EscapeDataString(InstanceToken)}/update-auto-read-message";
            ...
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    Console.WriteLine("Leitura automática atualizada com sucesso");
...
func main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    url := fmt.Sprintf("https://api.z-api.io/instances/%s/token/%s/update-auto-read-message", instanceId, instanceToken)
    ...
    if resp.StatusCode >= 200 && resp.StatusCode < 300 {
        ...
        // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
        fmt.Println("Leitura automática atualizada com sucesso")
...
if ($error) {
    ...
    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    echo "Leitura automática atualizada com sucesso\n";
...
# ⚠️ SEGURANÇA: Sempre use HTTPS
url = URI("https://api.z-api.io/instances/#{CGI.escape(instance_id)}/token/#{CGI.escape(instance_token)}/update-auto-read-message")
...
begin
  ...
  if response.code.to_i >= 200 && response.code.to_i < 300
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    puts 'Leitura automática atualizada com sucesso'
    puts result.to_json
  else
    # ⚠️ SEGURANÇA: Não exponha detalhes sensíveis em logs
...
// ⚠️ SEGURANÇA: Sempre use HTTPS
guard let url = URL(string: "https://api.z-api.io/instances/\(instanceId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? instanceId)/token/\(instanceToken.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? instanceToken)/update-auto-read-message") else {
    print("URL inválida")
    exit(1)
}
        ...
        if let data = data {
            do {
                if let result = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                    print("Leitura automática atualizada com sucesso")
                    print(result)
                }
            } catch {
...
# ⚠️ SEGURANÇA: Sempre use HTTPS
$url = "https://api.z-api.io/instances/$([System.Web.HttpUtility]::UrlEncode($instanceId))/token/$([System.Web.HttpUtility]::UrlEncode($instanceToken))/update-auto-read-message"
...
try {
    ...
    # ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
    Write-Host "Leitura automática atualizada com sucesso"
...
```http
PUT https://api.z-api.io/instances/SUA_INSTANCIA/token/SEU_TOKEN/update-auto-read-message HTTP/1.1
Host: api.z-api.io
Content-Type: application/json
...
int main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    std::string url = "https://api.z-api.io/instances/" + instanceId + "/token/" + instanceToken + "/update-auto-read-message";
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                std::cout << "Leitura automática atualizada com sucesso" << std::endl;
...
int main() {
    ...
    // ⚠️ SEGURANÇA: Sempre use HTTPS
    char url[512];
    snprintf(url, sizeof(url), "https://api.z-api.io/instances/%s/token/%s/update-auto-read-message", instanceId, instanceToken);
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            ...
            if (responseCode >= 200 && responseCode < 300) {
                // ⚠️ SEGURANÇA: Não logue tokens ou dados sensíveis
                printf("Leitura automática atualizada com sucesso\n");
                printf("%s\n", responseData);
            } else {
                printf("Erro HTTP %ld\n", responseCode);
            }
...
|-------|------|-----------|
| `value` | boolean | Confirmação do estado da configuração (`true` se ativado, `false` se desativado) |
...
## <Icon name="Monitor" size="md" /> Painel Administrativo {#painel}

Você também pode configurar a leitura automática através do painel administrativo:

![Configuração de leitura automática](/img/auto-read.jpeg)

---

## <Icon name="Info" size="md" /> Observações {#observacoes}

- A leitura automática se aplica a **todas as mensagens recebidas**
- Quando ativada, as mensagens serão marcadas como lidas automaticamente
- Para desativar, envie `{"value": false}` na requisição
